...
- A PRD whose `**Status:**` line starts with `Accepted` but whose Linear description still says "PRD: TBD" or has no PRD line at all is a hit — same class of gap as LOC-44 on 2026-07-14 (project description said "PRD: TBD" for six hours after the PRD was actually approved and committed). Current Passenger PRDs use bold `**Status:** Accepted …` (not plain `Status: Approved`) — match on that.
...
### B. Label hygiene
- `gate:prd-approved` present iff the matched PRD's `**Status:**` line starts with `Accepted`. Remove it if a PRD got reverted to Draft (rare — don't leave a stale approval label sitting on an issue).
...
- **Stranded worktree branches — finished-looking work nobody can see.** In both repos, `git worktree list` and `git branch --list 'claude/*'`; for each, `git merge-base --is-ancestor <branch> main`. A branch that is *not* an ancestor of `main` is a hit: report it with its commit subjects and the files it touches. Two branches carrying the same files is a second, separate hit — that's duplicated work, not just undelivered work. This is a **dispatch chief-of-staff** item, never a fix-silently one; merging another session's isolated branch is not yours to decide. Evidence: on 2026-08-02 three regeneration commits sat on two `claude/*` branches, two of them byte-identical to each other, while the files they regenerated were still stale on `main` (`CLAUDE.md` rule 10).
...
The git lifecycle has more stages than Linear's status column: `… → qa → acceptance → aviran-review → done`. When a task's worklog records a product **ACCEPT** and moves it to `aviran-review`, it's done being worked and is waiting only on Aviran's sign-off. That accept→aviran-review transition isn't emitted to Linear automatically, so accepted work can silently sit in `In Progress` (exactly what happened to LOC-41, LOC-50, LOC-51 on 2026-07-17 — all three product-ACCEPTED and at `aviran-review`, all three still showing `In Progress` on the board days later).
- **First confirm the target state exists** — `list_issue_statuses` for the passenger team. Historically this mapped to Linear `In Review`; as of 2026-08-03 the passenger team has no `In Review` workflow state, so this check is currently inapplicable — don't invent a state name. If a review-equivalent state exists, use it; if none does, skip the autofix and just flag accepted-but-`In Progress` issues in the digest instead (tier E, judgment).
...
- **If a review-equivalent state exists, fix directly via `save_issue`** (`state: <that state>`) — this is the one mechanical accept→aviran-review sync point, same class as a PRD-link fix: it follows deterministically from the git state, no judgment. **Do not** move it to `Done` — Aviran's review is the gate to Done, and only he clears it.
...
- Also flag (same tier) any PRD over its line budget without an explanation: >100 lines with no `Decisions log` (or nearby note) explaining why. As of 2026-08-03 most current Passenger PRDs run 100–140 lines, so expect this to fire often until product either raises the budget or backfills Decisions-log notes — don't hand-maintain a fixed exempt-path list here (the old Locali-era one rotted completely once every named path stopped existing); the Decisions-log carve-out is the only exemption mechanism. This is a light-touch size check, not a rewrite — just surface it.

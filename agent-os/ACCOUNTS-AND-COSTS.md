...
| **buzz** | The founders' channel — run summaries and gate pings. Replaces hilos (retired 2026-08-04). | 4 | Not researched — buzz is a local Nostr-relay CLI already installed on Aviran's machine; pricing/hosting model for a shared multi-founder relay isn't established yet. | **Not set up.** Needs a relay, an owner-approved agent identity for Chief, and the founders' pubkeys before anyone can use it. |
...
- [x] hilos retired 2026-08-04 — daemon stopped, `~/.hilos/agent.json` deleted locally. **Still open: revoke the token on hilos.sh's own panel** (can't be done from here — Aviran, sign in and revoke).
- [ ] Revoke the buzz identity/private key for whoever's leaving, once buzz accounts exist — tokens/keys post as that person
...
- hilos pricing (retired 2026-08-04) was read directly from `hilos.sh/pricing`, 2026-07-26 — kept for history, no longer relevant. buzz pricing/hosting: not researched.

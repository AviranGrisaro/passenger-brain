...
Direct dispatch to a specific agent still stands for quick, low-stakes, single-agent asks that don't create new work — "qa, verify X" is fine. Anything that creates new work, writes to Linear, or crosses domains goes through `chief`. Favorite-agent affinity (above) is about whose output you personally review — it isn't a way to route real work around chief.

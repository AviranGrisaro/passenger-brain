...
| T-030 | Scope live-events ingestion pipeline — new V1 launch-blocking dependency (Linear `PAS-5`) | backlog | data-engineer | 2026-07-29 | Founder-direct, live chat, 2026-07-29 (PROGRESS.md). Aviran moved the live events layer from a Phase 2 candidate into V1, alongside heat + tag — **but launch-blocking**: V1 doesn't ship until this pipeline works. Not yet authorized in `strategy/passenger-strategy.md` — chief-of-staff has a proposed redline pending Aviran's sign-off (not applied). Design side already updated in `design/ux-flows.md` §8a. `data-engineer` scoping ask: sourcing options + realistic freshness/coverage, rough build-cost/timeline, and an explicit call on whether this fits the current Phase 1 timeline. **Note:** Linear's `Passenger V1` project doesn't exist yet in the reset workspace — `PAS-5` was filed at the team level (`Passenger`/`PAS`), no project attached; create/attach the project when one exists. |

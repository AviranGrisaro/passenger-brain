...
- **CI** — `passenger-code/.github/workflows/ios-build.yml` exists (added 2026-08-03, PAS-41) and triggers on every push/PR to `main`, on GitHub's free macOS runners. It's build-only (`CODE_SIGNING_ALLOWED=NO`, `xcodebuild build` against the `Passenger` scheme) — signing gets added later, see below. **Confirmed live and green**, not just locally-equivalent: `gh run list --workflow=ios-build.yml` shows a real run (`31026972363`, triggered by push to `main`, 2026-08-05T16:47:52Z) that completed `success` in 1m17s — watched via `gh run view 31026972363`, not assumed. This closes the loop the 2026-08-03 entry left open ("no agent session in this workspace can observe a live GitHub Actions run"); that gap is gone as of this check (2026-08-07, PAS-41).

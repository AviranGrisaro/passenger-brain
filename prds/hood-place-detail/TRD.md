...
**The answer:** the nav icons render as separate independent buttons side by side, not a fused/joined bar; the switching behaviour is unchanged. Aviran, verbatim — *"yes, separate icons, same switching behavior."* Live chat, **relayed** via `chief-of-staff` (`PROGRESS.md`, 2026-07-31 FOUNDER-DIRECT STUB); not heard first-hand here. A second, differently-worded relay of the same answer exists and is not merged with it — see `time-slider/TRD.md` §8 D6 and `PROGRESS.md`'s 2026-07-31 CONCURRENCY FLAG. Both agree on the substance this decision rests on.

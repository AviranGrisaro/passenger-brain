...
- **Data sourcing (added 2026-07-30, standing rule).** Neither table has rows and neither is authored by this PRD. The `hoods.blurb` column does not exist in migration `001` and every blurb is a person-authored artifact — [`prds/hood-dataset/`](../hood-dataset/hood-dataset.md) req 4. The `places` table does not exist at all; its rows, its two-value category constraint, and its coordinates→`hood_id` containment check are [`prds/places-dataset/`](../places-dataset/places-dataset.md) reqs 1–2. Both own the **launch** dataset. ~~Both are hard upstream of this feature: req 2 and req 6 are unfalsifiable against an empty dataset.~~ **Corrected 2026-07-31:** neither is upstream of *building and accepting* this feature — see the Build-Phase-1 bullet below. Both remain upstream of launch readiness.
- **Build-Phase-1 data need (added 2026-07-31).** Under the `BOARD.md` V1 build-phase sequencing, this feature ships client-only with no backend, so its Phase-1 data is a **checked-in bundled fixture, not a Supabase read** — and the fixture is the deliverable that makes reqs 1–7 falsifiable. Two files, both in `passenger-code/Passenger/Resources/`:
  - **`places-tel-aviv.json`** — does not exist; must be authored. Real Tel Aviv place names, both categories present, each place's coordinate falling **inside** the bundled polygon its `hood_id` names (the bundled Hoods are placeholder rectangles, so coordinates are snapped, not real). Floor for a demo that reads as working: ≥8 places across ≥2 Hoods, plus one Hood deliberately left with **zero** places and one deliberately left with **no blurb**, so req 2's two empty branches are observable rather than asserted. Declares itself provisional in-file; T-042 step B5 replaces it wholesale at Build Phase 2.
  - **`hoods-tel-aviv.json`** — exists at `schemaVersion` 1 with **no `blurb` field**. Without it, Phase 1 can only demo req 2's no-blurb branch, and the blurb is half of this feature's own authorizing strategy line. Phase 1 therefore needs hand-authored provisional blurbs in this file. The field, the file and the generator are [`hood-dataset`](../hood-dataset/hood-dataset.md)'s (its TRD §3.5, C1–C3) — **not this PRD's to re-home** — so this is a sequencing ask on that task, not a second bundled home for `blurb`.
  - **[ASSUMPTION]** the numeric floors above (≥8 places, ≥2 Hoods, one empty, one blurb-less) are product judgment for a demo that doesn't read as a toy, not a founder-stated threshold. Cheap to overturn; the empty/blurb-less pair is the part worth keeping either way.

...
4. **Hand-curated blurb, authored per Hood.**
   ...
   - [ ] **Recorded unmet at acceptance 2026-08-01, carried not waived.** All 24 shipped Hoods carry `blurb: null` in the authored source and the DB seed. One `[PROVISIONAL]`-marked string was hand-added to `florentin` in the iOS bundle so req 2's blurb branch has something to render — a Phase-1 demo device, tolerated only while the marker stays visible in the rendered text. No build seen outside the team may ship a `[PROVISIONAL]` blurb; this bullet is met when a person has authored real copy.

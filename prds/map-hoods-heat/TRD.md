...
**Amended 2026-07-30, post-`qa` Major finding.** The pseudocode above only covers the tap-initiated grant path. The auto-scheduled cold-open system prompt (§8's permission timing) can *also* transition `.notDetermined → authorized` with no near-me tap in the loop at all — `qa` caught this in a live repro (camera never recentered after granting via the system prompt directly) and it shipped as a real gap in the first build. The correct handling, now built and verified (`ios-developer` commit `f31816c`, `ios-code-reviewer`-approved `555597a`, `qa`-reconfirmed `f466e36`): a `.onChange(of: locationStore.authorizationStatus)` observer, gated by a pure predicate (`isNewGrant(from:to:)`) that fires the same recenter **only** on a genuine not-authorized→authorized transition — never on the lateral `authorizedWhenInUse`↔`authorizedAlways` move, never on a same-status re-render. This is a second, independent trigger for the same recenter action D2 already specifies, not a new requirement — both paths converge on the same `camera = .userLocation(fallback:)` assignment.

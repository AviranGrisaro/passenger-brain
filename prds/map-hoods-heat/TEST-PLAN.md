...
| 17 | **Granted: the map recenters on the user** | Req 6 | Re-verified 2026-07-30 against `passenger-code` commit `f31816c` (fix) / `555597a` (passenger-brain, code-review). Fresh install, `simctl location set` to Jerusalem (31.7683/35.2137), auto-scheduled system prompt appears with zero near-me tap. **Literal tap attempted first, same as the original repro:** 3 separate input attempts across 2 different alert buttons ("Allow While Using App" via `tap` and `touch_path`, "Allow Once" via `tap`) all left the system alert unresponsive, while the status-bar clock visibly advanced between screenshots (22:34→22:35), confirming live, non-stale captures and a genuine input-delivery wall to the SpringBoard-hosted alert — same tooling limitation `ios-code-reviewer` disclosed, not a fresh finding. Fell back to `simctl privacy grant location`, the same TCC-level equivalent `ios-code-reviewer` used, which reaches the app through the identical `CLLocationManagerDelegate` → `.onChange(of: authorizationStatus)` path a real tap would. | **PASS** — camera visibly recentered off the Tel-Aviv-wide default onto Jerusalem (The Knesset, Rehavia's Menahem Ussishkin/Ibn Ezra Road, Katamon) with the blue user-location arrow rendered, immediately after the grant. Finding A is closed. |
...
## Finding A — CLOSED 2026-07-30

**Original finding (2026-07-30, this pass): Req 6's "Granted: the map recenters on the user" was not satisfied on the app's own default flow.** Fixed by `ios-developer` (`f31816c`, new `.onChange(of: locationStore.authorizationStatus)` + `isNewGrant(from:to:)` predicate), reviewed by `ios-code-reviewer` (APPROVE WITH MINORS, `555597a`), re-verified by `qa` 2026-07-30 — see case 17 above. Also spot-checked for regressions in the surrounding permission-flow behavior (not re-running the full original pass, per dispatch scope): denied state still shows no recenter + disabled near-me icon (correct, `isNewGrant` returns false for notDetermined→denied); re-granting after a denial correctly re-fires the recenter (not a one-shot-ever bug scoped to first launch only). Original finding text preserved below for the record.
...
**FAIL (2026-07-30, original pass) — one Major finding (Finding A, req 6 recenter-on-grant), one route back to `build` for `ios-developer`.** Everything else in this pass is PASS, matches or exceeds what `code-review` already established, and the 4 carried-forward should-fix items remain correctly triaged as non-blocking. Security and physical-device verification remain explicitly tracked as unverified-from-any-sandbox, not silently dropped.

## Re-verification verdict — 2026-07-30, PASS

**PASS.** Finding A is closed (case 17 above, `f31816c`/`555597a`). Full test target re-run in one invocation (`PassengerTests` + `PassengerUITests`) — 35/35 unit + 1/1 UI, 0 failures, cold-open signpost 0.494s avg. Live recenter-on-grant re-verified via `simctl privacy grant location` after the literal system-alert tap hit the same disclosed tooling wall `ios-code-reviewer` hit. Spot-checked, not re-run in full: denied state (no recenter, disabled icon — still correct) and repeat-grant-after-denial (recenter still fires) both regression-free. T-031 moves to `acceptance`.

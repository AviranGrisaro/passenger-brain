...
3. **Hoods.**
   - [ ] Each Hood is a named polygon; Tel Aviv ships dozens of them, not thousands (decision #12's bound).
     ...
     - **Now spec'd: [`prds/hood-dataset/hood-dataset.md`](../hood-dataset/hood-dataset.md)** (T-040 / `PAS-17`), written 2026-07-30 under the standing data/schema rule. That PRD carries this bullet, the non-overlap validator the bullet below asks for, and the three Hood columns migration `001` lacks (`blurb`, `is_tourist_trap`, `designated_for_progression`).
...
- **Data sourcing (added 2026-07-30, standing rule).** This PRD defines the density contract and the *shape* of `hoods`; it does not source the geometry. Real polygons, names, blurbs and the three attribute columns are [`prds/hood-dataset/`](../hood-dataset/hood-dataset.md)'s deliverable. `hood_density` values are synthetic and already generated (migration `002`); a live popular-times source is out of V1 (strategy).
...
| 2026-07-30 | Data-sourcing bullet added to Technical design; req 3's carried gap now points at its own PRD | Standing rule, founder-direct 2026-07-30 (`agent-os/PROGRESS.md`): a data need substantial enough to be its own deliverable gets its own PRD. `hood-dataset` written the same day; this PRD's own scope and requirements are unchanged |

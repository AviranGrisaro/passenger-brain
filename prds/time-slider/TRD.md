...
**Task:** T-032 · **Linear:** `PAS-15` (§9 amendments carried at `PAS-51`; v6 re-scope at `T-077`/`PAS-51`) · **Status:** v6.1 — §9 amendment + one new build step (C16), no `trd-review` re-run owed
**Owner:** architect · **Date:** 2026-07-30 · **Revised:** 2026-08-07 (v6.1)
...
| **4** Session persistence across every dismissal path | **[rewritten at v6 — `.heat` no longer exists]** `MapChromeState.toggle(.search)` → `toggle(.places)` → `toggle(.search)`, plus the new intra-surface path the merge created: Hour segment → Search segment → Hour segment **without** dismissing the overlay; and set-by-edge → read-by-slider | `selectedHour` unchanged across every switch above; the value set by an edge drag is the value the Hour segment shows, and vice versa. **[v6 audit] positive control** — this is an *unchanged* claim and passes identically if the hour never moved at all: the value under test is first driven to a **non-default** hour (`selectedHour != 0`, recorded) before the first switch, so "unchanged" is a claim about something that had changed | unit + manual | C2, C11 |
| **5** Hour readable as a number; explicit "now" mark; never colour alone | `HourFormat.readout` strings; the rendered "now" mark | `offsetLabel` is non-empty at all 13 offsets; "Now" at 0; a midnight-crossing case sets `isNextDay`; the "now" mark differs from an ordinary stop in **shape** — **[v5 audit] restated as a geometric observable**, since "differs in shape" is not something a test can see: with the colour catalog forced to greyscale, capture the "now" mark's frame and an ordinary stop's frame, record both, and assert they differ in `width` **or** `height` by ≥2pt. Both frames must be non-zero first, or the row is unrun | unit + **rendered** (screenshot or UI test, frames recorded; blocked → BLOCKED, not passed) | C3, C5 |
| **5b** The Hour segment's readout renders **unoccluded and uncut** — §2.3's z5 rule made checkable, in its content form. PRD v9 req 5's rendered-legibility bullet; new at v4 (T-055), rewritten at v5, **re-targeted at v6 (T-077) after `HeatModalCard` was deleted** | With **`SearchOverlay` open on the Hour segment** — entry path is nav-row `SearchButton` (label `"Search and hours"`, `PAS-75`) → the `"Hour"` picker segment, **two taps, there is no heat button** — (a) at the **default** text size, the rendered frames of `hourSegmentCard` (the fixed-fraction card frame; identifier added by **C16**), `mapNavRow`, `hourReadout` and `hourSlider`; (b) the identical capture at the Hour segment's own Dynamic Type ceiling — `SearchOverlay.maxDynamicTypeSize`, `.accessibility3` today (`SearchOverlay.swift:52`, the clamp carried over verbatim from the deleted card and now scoped to `hourContent`) — driven by **C15**'s `-uiTestDynamicTypeSize accessibility3`, *not* by `-UIPreferredContentSizeCategoryName` or `simctl ui content_size`, both confirmed non-functional on this toolchain; (b2) a third capture requested at **AX5** whose recorded **`hourReadout` and `hourSlider`** frames are **equal** to (b)'s — that equality *is* the ceiling binding, and it is the only claim about AX5 this design permits. **[v6] Asserted on the content elements, never on the card:** the card's height is `0.45 × screen height` at every text size (`SearchOverlay.swift:56`), so a card-frame equality is true whether or not the clamp exists and cannot fail; (c) `hourReadout.label` in each capture at an hour where `HourFormat.isNextDay` is `true` — pinned by **C15**'s `-uiTestNow` fixed instant with the slider at +12h, never by whatever time the suite happens to run at | **Counts and non-emptiness first, then the geometry.** All four elements exist and each reports `frame.width > 0` **and** `frame.height > 0`, values recorded — frames that never rendered satisfy every non-intersection and containment claim below, and that is the exact way this row could pass while the requirement fails. Any zero-sized frame → the row is **unrun, not passed**. Then, **in captures (a) and (b) independently**: **(i) occlusion, restated on the content [v6]:** `hourReadout.frame.intersects(mapNavRow.frame) == false` **and** `hourSlider.frame.intersects(mapNavRow.frame) == false`. **Do not assert card-vs-nav-row non-intersection or any gap between them** — the card is full-width and flush to the true bottom edge (`.ignoresSafeArea(edges: .bottom)`, T-079/`PAS-73`) and `MapNavRow` renders above it by z-order deliberately (T-078/`PAS-60`), so v5's card/nav-row separation would now **fail a correct build**. Nav-row occlusion always was a claim about the content: F1 was a truncated "next day" pill, not a truncated card. **(ii) containment, which replaces v5's bottom-gap check and is the failure mode this merge introduced [v6]:** `hourSegmentCard.frame.contains(hourReadout.frame)` and `hourSegmentCard.frame.contains(hourSlider.frame)`, and both frames lie fully inside `app.windows.firstMatch.frame`. `hourContent` is a top-aligned `VStack` inside a **fixed** 45% frame with no `.clipped()`, so growth at AX3 no longer lengthens the card — it pushes the slider toward and past the bottom edge, off-screen. Nothing in v5 could have caught this, because v5's card grew with its content. **(iii) the P0 control is still a control at AX3 [v6]:** `hourSlider.frame.height >= 44` in **both** captures — req 6(b)'s ≥44pt stops being proved by the frame constant alone once the container stops growing. **And the wrap check, which is what F2 was — restated for a combined element [v6]:** `HourReadout` is one combined accessibility element (`.accessibilityElement(children: .combine)` + an overriding `.accessibilityLabel`, `HourReadout.swift:29-34`), so the pill, the numeral and the clock label have **no separately queryable frames**; v5 asked for the pill's own height and no test can obtain it. **Do not un-combine the element to make this measurable** — that trades a P0 VoiceOver behaviour (one spoken unit) for test convenience. Check the combined row instead: `hourReadout.frame.height` in capture (b) is **less than twice** its height in capture (a) scaled by the AX3/default text-size ratio — i.e. it still occupies one line — and, per (ii), is still contained in the card. Both numbers recorded. **Positive control** (standing rule 2), **corrected at v6**: in each capture `hourReadout.label` contains **`"+12 hours"`** and **`"next day"`** — the tokens `HourFormat.voiceOverValue` actually assembles. **Not `"+12h"`**: `.accessibilityLabel` overrides the visual text, so the short visual form is never what XCUITest reads, and v5 demanded a control that could only fail (`ios-developer` hit this at build). Deterministic because C15 pins the clock; before v5 it fired only after local noon. **This control does not detect occlusion and must not be read as if it did** — `XCUIElement.label` returns the whole string for a fully covered `Text`, which is precisely why F1 survived `qa`. Its only job is to prove the elements rendered at all, so (i)–(iii) are claims about something. If the pill's token is missing at an hour where `isNextDay` is `true`, the row is unrun. **[ASSUMPTION] retired at v6:** v5's 8pt card/nav-row floor is deleted along with the clause it qualified; this row now asserts no fixed-distance constant anywhere | **UI test on a rendered app** — `PassengerUITests/SearchHourSegmentInteractionTests.swift`, which already carries F1's default-size half and whose header explicitly discloses this row as an owed follow-up. **Not** unit, **not** source review, **not** `exists`. `SearchOverlayHourGuardTests` (`PassengerTests/`, the successor to `HeatModalCardLayoutGuardTests`) is a source-grep **regression backstop only** and explicitly does **not** discharge this row. Text size and clock come from **C15**'s launch arguments, so "the simulator won't change content size" is not a blocker here. If C15's `.environment(\.dynamicTypeSize, …)` propagation into `SearchOverlay` does not take effect, that is a **BLOCKED** disclosure written into this row — **not** grounds to fall back to the grep. **And no sub-check above may share a test method with an assertion known to fail (standing rule 5, ratified 2026-08-07).** The live case is sub-check **(iii)**: `hourSlider` renders 31pt — disclosed, out of scope here, owned by **T-081/`PAS-76`** — and inside `assertHourSegmentCapture` it aborts capture (a) before capture (b) is ever taken, which is exactly how (i)/(ii)/the wrap check/the positive control were reported as passing at `1021428` without running. **(iii) moves to its own test method**, red and tracked, while the rest run at both text sizes; and the report on this row states, per sub-check **and per capture**, *that it executed* — a count of failure lines is not that statement | **C16** (new at v6) — one step now owns both halves, which is itself the fix: at v5 the relationship was split across C2 (nav row) and C5 (card), and neither build step could check what only their composition produces. C16 adds the missing `hourSegmentCard` identifier and builds the suite |
| **6** VoiceOver discrete steps; ≥44pt on the button path; Dynamic Type; contrast | (a) VoiceOver swipe on `hourSlider`; (b) the `Slider`'s frame height, **at the default size (unit) and rendered at the ceiling (5b(iii)) [v6]**; (c) **[rewritten at v5, re-targeted at v6]** rendered readout at the **Hour segment's** Dynamic Type **ceiling** (`.accessibility3`), driven by C15's launch argument, plus a request at `AX5` proving the clamp binds — measured on the readout and slider frames, never on the fixed-fraction card frame; (d) `ContrastRatio` over the token pairs. **All rendered sub-checks are reached through the two-tap entry path (`SearchButton` → `"Hour"` segment); there is no heat button [v6]** | (a) one hour per swipe, value spoken; (b) ≥44pt in the unit check **and** in row 5b's capture (b) — the unit constant alone stopped proving the rendered result when the container became fixed-height **[v6]**; (c) **discharged by row 5b's (b)/(b2)/(iii) captures — same observables, same numbers, not re-derived here.** "No clipping or truncation" is deleted as a pass condition: it is not observable through `.label` or `.exists`, and stating it as if it were is how this row would pass while the requirement failed; (d) `MutedOnSurface` on `Surface` and on `PillSurface` ≥4.5:1, and `NowTick`/`SliderFill`/`EdgeRail` on `Surface` ≥3:1, all in both `UIUserInterfaceStyle`s — and the native thumb/rail are **not** asserted (§4.12) | unit + manual + **rendered for (b)/(c), via row 5b** — and because (b)'s rendered half *is* row 5b sub-check (iii), it inherits that row's standing-rule-5 requirement: own test method, per-sub-check execution reported **[v6.1]** | **C4** (the ≥44pt frame), C9, C12, **C15**, **C16** |
...
Ordered. **Every step is `[iOS]`.** No `[Backend]`, no `[Algo/Data]` — see §1. C1–C9 are v1's, unchanged in scope; C10–C14 are new at v2; **C15 is new at v5** and **C16 is new at v6** — neither changes feature behaviour, and both exist only for §9's rendered rows. **C1–C15 are all built and shipped** (`passenger-code` through `967b7c2`); **C16 is the only outstanding step in this document**, owned by `ios-developer`.
...
| **C16** | **Re-target the row 5b / 6(c) rendered check at `SearchOverlay`'s Hour segment [new at v6, T-077/`PAS-51`].** Three parts, all `passenger-code`: (1) add `.accessibilityIdentifier("hourSegmentCard")` to the fixed-fraction card frame in `SearchOverlay.body` — the one identifier the T-078 merge dropped, and the container frame row 5b's containment check needs; (2) build the rendered AX3/AX5 suite in `PassengerUITests/SearchHourSegmentInteractionTests.swift` exactly per row 5b, launching with **C15**'s `-uiTestDynamicTypeSize` and `-uiTestNow`, and replace that file's header disclosure ("left for a follow-up story rather than invented here") with the tests themselves; (3) comment-only — `UITestOverrides.swift`'s doc comment still names `HeatModalCard`; retarget it to `SearchOverlay`'s Hour segment. **Do not touch either override's behaviour.** **Three things this step must not do:** re-add a card-vs-nav-row non-intersection or separation assertion, assert the card's `maxY` sits above the safe-area bottom (both contradict T-078/T-079's shipped shape and would fail a correct build), or un-combine `HourReadout`'s accessibility element to obtain per-element frames (that regresses a P0 VoiceOver behaviour). If C15's `.environment(\.dynamicTypeSize, …)` does not propagate into `SearchOverlay`'s Hour segment, report **BLOCKED** on row 5b — the source grep in `SearchOverlayHourGuardTests` does not discharge it | **[iOS]** |

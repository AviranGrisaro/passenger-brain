...
**Task:** T-032 · **Linear:** `PAS-15` (§9 amendments carried at `PAS-51`; v6 re-scope at `T-077`/`PAS-51`; v7 retirement at `T-090`/`PAS-88`) · **Status:** v7 — §9 amendment (row 5b retired; rows 2b/4/5/6/7d re-targeted or narrowed), C16 retired, C17 added, **D4 and D5 amended**, **§2.3's and §10's co-presentation claim restated on its live mechanism**, no `trd-review` re-run owed
...
**What changed at v7 (2026-08-08) — T-090/`PAS-88`: the in-modal hour path was deleted by founder order, so the rows that measured it are retired, not re-targeted again.** No decision, contract or scope in §2–§8 or §10 moves; **§9 and §11 do**. `trd-review` is not re-run — this changes gates and build steps, not a design, the same posture as v4/v5/v6.

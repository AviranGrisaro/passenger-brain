...
**Task:** T-032 · **Linear:** `PAS-15` (§9 amendments carried at `PAS-51`) · **Status:** v5 — §9 amendment + one new build step (C15), no `trd-review` re-run owed
**Owner:** architect · **Date:** 2026-07-30 · **Revised:** 2026-08-07 (v5)
...
| **5b** The readout renders **unoccluded** — §2.3's z5 rule made checkable. PRD v9 req 5's rendered-legibility bullet; **new at v4, T-055** | With the heat modal presented: (a) at the **default** text size, the rendered frames of `HeatModalCard`, of `MapNavRow`, and of the three readout elements individually — offset numeral, clock label, "next day" pill; (b) **[rewritten at v5]** the identical capture at **the card's own Dynamic Type ceiling** — `HeatModalCard.maxDynamicTypeSize`, `.accessibility3` today — driven by **C15**'s `-uiTestDynamicTypeSize` launch argument, *not* by `-UIPreferredContentSizeCategoryName` or `simctl ui content_size`, both confirmed non-functional on this toolchain. The card is *sized to content* (§2.3 z5), so its height is a function of text size and a default-size gap says nothing about the largest one. **v4 asked for a capture at AX5; that became unsatisfiable when the card gained its ceiling** — a clamped card never renders at AX5, so the honest check is the ceiling, plus (b2) below; (b2) a third capture requested at **AX5** whose recorded frames are **equal** to (b)'s — that equality *is* the ceiling binding, and it is the only claim about AX5 this design permits; (c) the rendered `label` of the pill and of the numeral in each capture, at an hour where `HourFormat.isNextDay` is `true` — pinned by **C15**'s `-uiTestNow` fixed instant (e.g. 23:00 local, offset +12h), never by whatever time the suite happens to run at | **Counts and non-emptiness first, then the geometry.** All five elements exist and each reports `frame.width > 0` **and** `frame.height > 0`, with the values recorded — two frames that never rendered satisfy every non-intersection claim below, and that is the exact way this row could pass while the requirement fails. Any zero-sized frame → the row is **unrun, not passed**. Then, **in each of the two captures independently**: (i) `HeatModalCard.frame.intersects(MapNavRow.frame) == false`, and stronger, `MapNavRow.frame.minY − HeatModalCard.frame.maxY ≥ 8pt` — a separation a person can see, not a hairline; (ii) the same non-intersection asserted **separately for each of the three readout elements** against `MapNavRow.frame`, because the card can clear the nav row while a pill drawn at its edge does not; (iii) `HeatModalCard.frame.maxY` is strictly less than the safe-area bottom — `bottom: 0` is what §2.3 forbids by name, so it is falsified directly rather than inferred from the gap. **And the wrap check, which is what F2 was [new at v5]:** mid-token wrapping is invisible to `.label`, so it is checked as height — in capture (b), the pill's `frame.height` is **less than twice** its height in capture (a) scaled by the text-size ratio, i.e. it still occupies one line, and the pill's frame is fully **contained** within `HeatModalCard.frame` (a wrapped or overflowing pill escapes the capsule, which is exactly what the F2 REJECT described). Both numbers recorded. **Positive control** (standing rule 2): in the same capture the pill's `label` reads `"next day"` in full and the numeral reads `"+12h"` in full — and this control is now *deterministic*, because C15 pins the clock; before v5 it fired only when the suite ran after local noon, so the row reported a pass while its own control was absent for half of every day. **This control does not detect the occlusion and must not be read as if it did** — `XCUIElement.label` returns the whole string for a fully covered `Text`, which is precisely why F1 survived `qa`. Its only job is to prove the elements rendered at all, so that (i)–(iii) are claims about something. If the pill is missing at an hour where `isNextDay` is `true`, the row is unrun. **[ASSUMPTION] on the 8pt floor:** §2.3 requires the anchor distance be *fixed and non-zero* and names no number; the constant is `ios-developer`'s at build. If a different fixed distance is chosen deliberately, this floor moves to it — what may not move is that the gap is fixed, positive, and asserted | **UI test on a rendered app** (or on-device with frames read from the accessibility hierarchy). **Not** unit, **not** source review, **not** `exists` — none of the three can see an overlap, and all three passed this build. `HeatModalCardLayoutGuardTests`' source-string assertions are a **regression backstop only** and explicitly do **not** discharge this row [stated at v5, after `PAS-51` found them standing in for it]. Text size and clock come from **C15**'s launch arguments, so "the simulator won't change content size" is not a blocker here. Genuinely blocked environment → **BLOCKED**, reported as unrun, and disclosed in this row rather than silently substituted | **C2, C5** — C2 wires `MapNavRow` in at z7, C5 builds the card and readout; each owns half of a relationship neither one alone can check, which is why no build step caught it. Build note, not a contract change: both views need an `.accessibilityIdentifier` for their frames to be queryable — add it in whichever step lacks it |
| **6** VoiceOver discrete steps; ≥44pt on the button path; Dynamic Type; contrast | (a) VoiceOver swipe on `hourSlider`; (b) the `Slider`'s frame height; (c) **[rewritten at v5 — same defect as row 5b]** rendered readout at the card's Dynamic Type **ceiling** (`.accessibility3`), driven by C15's launch argument, plus a request at `AX5` proving the clamp binds; (d) `ContrastRatio` over the token pairs | (a) one hour per swipe, value spoken; (b) ≥44pt; (c) **discharged by row 5b's (b)/(b2) captures — same observables, same numbers, not re-derived here.** "No clipping or truncation" is deleted as a pass condition: it is not observable through `.label` or `.exists`, and stating it as if it were is how this row would pass while the requirement failed; (d) `MutedOnSurface` on `Surface` and on `PillSurface` ≥4.5:1, and `NowTick`/`SliderFill`/`EdgeRail` on `Surface` ≥3:1, all in both `UIUserInterfaceStyle`s — and the native thumb/rail are **not** asserted (§4.12) | unit + manual + **rendered for (c), via row 5b** | **C4** (the ≥44pt frame), C9, C12, **C15** |
...
Ordered. **Every step is `[iOS]`.** No `[Backend]`, no `[Algo/Data]` — see §1. C1–C9 are v1's, unchanged in scope; C10–C14 are new at v2; **C15 is new at v5** — it changes no feature behaviour and is required only by §9's rendered rows.

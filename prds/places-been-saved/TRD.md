...
| 10 | Which chrome bucket the Places button is in | found by reading `ux-flows.md` §2 against T-032's D1 | **Bucket 2, not the nav row** — `ux-flows.md` calls it "the 4th icon, separate from the 3 nav buttons." This corrects T-032's D1 note | §2.3, **D7** |
| 11 | A nav surface and a system sheet could be co-presented | found by reading the shipped `.presentationBackgroundInteraction` | Forbidden in both directions, structurally: opening `.places` calls `closeHood()`; leaving `.places` calls `closePlace()` | §4.6, **D8** |
...
### 2.4 Chrome placement — the Places button is not a nav-row button

`ux-flows.md` §2's Primary table is explicit: *"Saved-places icon (4th icon) — Persistent icon, **separate from the 3 nav buttons**."* The same doc's 2026-07-30 stacking addendum names "near-me (L44) and the Places icon (L46)" together as **bucket 2** — chrome that fades while a nav-row modal covers its position.

So the button and its surface sit in two different layers of T-032's z-order table:
...
| 4 | Bucket-2 chrome (`NearMeButton`, `HoodButton`, `SettingsHint`) | **`PlacesButton` joins this cluster** and fades with it |
| 5 | Modal card | `PlacesListOverlay` |
| 7 | `MapNavRow` | **Untouched.** T-036 adds no nav-row button |

**This corrects T-032's D1**, which anticipated that "T-036/T-037/T-038 add their own buttons to `MapNavRow`." For T-037 (Profile) and T-038 (Search) that is right — both are named nav buttons in `ux-flows.md` §2. For T-036 it is not.

**The accepted cost, stated rather than discovered (D7):** because the Places button is bucket-2 chrome, it fades while its own surface is open, so the list cannot be dismissed by re-tapping the icon the way a nav-row modal can. Three dismissal paths remain — drag handle, ✕, scrim tap — and the design spec §1 asserts this same behaviour deliberately. Flagged for `product`/`designer` at review; reversible by moving one button between two `overlay` blocks.
...
The shipped depth-1 sheet carries `.presentationBackgroundInteraction(.enabled(upThrough: .medium))`, so bucket-2 chrome *is* reachable while a Hood sheet or place modal is up. That makes both of these reachable states unless they are closed structurally:

- **Opening `.places` calls `router.closeHood()` first.** Otherwise tapping the Places button with a Hood sheet open presents the list *underneath* a system sheet, in a layer that can never be reached.
- **Leaving `.places` (any path, including a switch to another surface) calls `router.closePlace()`.** While `.places` is presented the scrim blocks map taps, so a depth-1 place modal can only have been opened from a list row; closing the list must not strand it. `router.placeDepth == 1` is the condition, derived — no `entryPath` field is added to `DetailRouter`, and `DetailRouter` itself is not modified by this task.
...
**Open the list.** Tap `PlacesButton` → `router.closeHood()` (D8) → `chrome.toggle(.places)` → scrim at z3, bucket-2 chrome (including the button itself) fades at z4, overlay slides in at z5, nav row stays live at z7. `PlacesListComposition.entries` runs once per render pass over already-loaded data.
...
`ux-flows.md` §2 says "4th icon, separate from the 3 nav buttons," and its stacking addendum lists the Places icon in bucket 2 by name. **This corrects T-032's D1's expectation** for this task only (T-037/T-038 really are nav-row buttons). Accepted cost: the icon fades under its own surface, so re-tapping it cannot dismiss the list; three other dismissal paths remain. **Flagged for `product`/`designer`** — cheap to reverse.

### D8 — A `NavSurface` and a system sheet are never co-presented, in either direction
Found by reading the shipped `.presentationBackgroundInteraction(.enabled(upThrough: .medium))`, which makes bucket-2 chrome tappable while a sheet is up — so both orderings are reachable states, not hypotheticals. Enforced by two calls (§4.6) rather than a flag, and it needs no `DetailRouter` change: `placeDepth == 1` under a presented `.places` already means "opened from a row," because the scrim blocks every other route to it.
...
| **6** Empty and offline states are plain, not errors | (a) empty saved set + empty fixture; (b) the app in airplane mode | (a) the empty state renders with icon, line, and a CTA that dismisses the list — not an error string, not a spinner; (b) every row renders identically and a save made offline appears immediately. **No offline banner is asserted — none ships (D5)** | unit + manual | C10 |

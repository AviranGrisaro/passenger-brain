...
**Amendments — 2026-08-04, documentation-only, no re-review cycle owed** (same treatment as the T-036/T-034 precedent). A1 and A2 resolve `ios-developer`'s two pre-C3 findings from its AGREE (`passenger-brain 8a006f7`); **A3 resolves `ios-code-reviewer`'s findings 2 and 3** from its AGREE (`dd68110`). None changes a requirement, a bound, or the privacy contract.
...
| **A3** | Two gaps in what the build is told to prove, both from `ios-code-reviewer` (`dd68110`): **(2)** §4.9's camera bottom-inset was justified by a single hardcoded iPhone 17 measurement (the `.medium` sheet at y 415–866), and a constant derived from one device silently breaks under Dynamic Type and iPad/landscape size classes. **(3)** §9 had **no row at all** for `RouteControls`' touch targets, VoiceOver labels or Dynamic Type — unlike every other interactive control in this app (`ProfileButton`, `SearchButton`), and these controls carry duration, distance and `via <Hood>` **text**, which is the thing that truncates. | **(2)** §4.9 now requires the inset to be read from the **rendered** sheet frame at runtime, names the mechanism and the no-constant rule, and specifies the one permitted re-fit; §9 row 12 gains a second run at an accessibility text size plus a grep that fails on any device constant in the fit path. **(3)** New §9 **row 15** — counts-then-∀ on 44×44 frames, VoiceOver labels that carry the *reason* a disabled control is disabled, and no-truncation at `AX5` checked on rendered geometry rather than by grepping `.lineLimit`. | §4.9, §9 rows 12 and **15** (new), §10 C5/C6/C7/C8, **D17** |
...
**Camera.** On first resolve, fit the camera to the union of both polylines' bounding rects, with a **bottom inset equal to the height of the sheet as actually rendered at that moment** — read at runtime, never a constant. Selecting the other route does **not** re-fit; the camera moves once, on resolve.

**The inset is measured, not hardcoded — amendment A3, 2026-08-04** (`ios-code-reviewer` finding 2, `passenger-brain dd68110`). `qa`'s measurement of the depth-1 `.medium` sheet at y 415–866 on an iPhone 17 (2026-08-04, T-036) is the **evidence that the inset is needed at all** — more than half the screen — and is **not** the value to ship. That number is one device, at one Dynamic Type size, in one size class; a `.medium` detent's height changes with Dynamic Type, with iPad and landscape size classes, and with any future detent change, and a route fitted against a stale constant lands under the sheet in exactly the cases hardest to notice in review.

C6 therefore reads the sheet's own rendered height. Preferred mechanism, in order: the sheet content's `GeometryReader`/`onGeometryChange` height published up to `MapScreen` (the presentation and the map already share `MapScreen` as their owner, so no new plumbing layer is needed); failing that, a `safeAreaInset`/`mapControlVisibility`-based fit that lets the layout system supply the value. **Two hard rules, either of which failing is a defect and not a tuning question:** the inset is derived from a measured frame every time the camera fits, and **no device-specific pixel constant appears in the routing or camera path at all** — §9 row 12 greps for one. If no rendered height is available at the moment of the fit (a resolve completing before first layout), fit with **zero** inset and re-fit once on the first height report, rather than substituting a guess; that is the only re-fit this feature permits, and it is a layout correction, not a second camera move in the sense §9 row 12 forbids.
...
**A fourth rule, ratified workspace-wide 2026-08-07** (`product` at T-077/`PAS-51` acceptance; `architect` ratification; canonical text in `architect.md` §9, lifted in here because `qa` reads this table on its own). (d) **A check sequenced behind a known-failing sibling is unrun, not passed.** Under `continueAfterFailure = false` — set by eight of this repo's ten `PassengerUITests` classes — and equally under `try #require`, `XCTUnwrap`, or an early `return` inside a shared assertion helper, the *first* failing assertion aborts every assertion after it in that method, while `xcodebuild` still reports **one** failure line for the whole method. A gate that counts failure lines reads that silence as a pass. **So no check in this table may share a test method with an assertion that is known to fail and is owned by another ticket** — move the known failure into its own method, where it stays visibly red and tracked while its siblings still execute. Any report on this table states, per sub-check, *that it executed*; a sub-check whose execution cannot be shown in the result bundle is **unrun**, exactly like a BLOCKED row. **`XCTExpectFailure` is not the fix** — it turns the method green and buries the tracked gap.

**Audit of this table against rule (d), same pass — one near-miss, no change owed.** This feature owns the workspace's other standing known failure: `ScenicWalkBundleInvariantTests.falseFlagCountAndTotal()` (Hood-dataset regeneration drift, parked at **T-076/`PAS-72`**). It does **not** suppress anything: it is a swift-testing `@Test` using non-aborting `#expect`, and its sibling `falseFlaggedHoodsAreMutuallyNonAdjacent()` is a separate function that runs regardless. Rule (d) still binds this file — if that failure is ever folded into a shared helper, or if either function's leading `try HoodCatalog.load()` starts throwing, the checks after it are unrun and must be reported as unrun.
...
| **12** | *(normative, §4.9)* The camera fits the route without hiding it under the sheet, **and the inset tracks the rendered sheet rather than a device constant** (amended by A3) | (a) rendered camera region after resolve, with the sheet presented at `.medium`; (b) **the same, re-run at `UICTContentSizeCategoryAccessibilityExtraExtraExtraLarge`, where the sheet renders taller**; (c) the source of the inset value | (a) both polylines' bounding rect is contained in the visible map region **and** the route's rect centre lies **above** the sheet's top edge (y < the sheet's `frame.minY`, measured from the rendered sheet, not assumed) — and the polyline count is asserted `≥ 1` first. Selecting the other route leaves the region's four components **byte-identical** (the camera moves once; §4.9's at-most-one re-fit on the first height report happens before any selection is possible and is not a second move); (b) the same containment and above-the-sheet condition holds — **and, asserted first, the sheet's rendered height at this size is strictly greater than at the default size** (otherwise the run proves nothing) **and the two runs' camera regions are not identical**, which is what a hardcoded inset would produce. Pick a route short enough that the fit is not clamped to a minimum span, and assert it isn't; (c) `grep -rn "415\|866\|451\|iPhone" Passenger/Routing Passenger/Map/MapScreen.swift Passenger/Map/RouteLayer.swift` → **0 hits** in any camera- or inset-related expression, positive control `grep -rn "inset\|GeometryReader\|onGeometryChange\|safeAreaInset" Passenger/Map/MapScreen.swift` → **> 0**. A device-specific constant anywhere in the fit path is a failure, not a simplification | UI test | C6 |
...
| **15** | *(normative, §4.10 — added by amendment A3)* `RouteControls` is reachable, speakable and legible, on the same terms as every other interactive control in this app | (a) rendered frames of the Fast control, the Scenic control and the Go button, with both plans present; (b) VoiceOver labels of the two route controls in `.success`, and of the Scenic control in each disabled case; (c) the same three controls at `UICTContentSizeCategoryAccessibilityExtraExtraExtraLarge`; (d) the disclosure line and the `via <Hood>` line at that size | **Counts first, then the ∀, per standing rule (a).** (a) exactly **3** interactive elements are found; **every one** reports `frame.height >= 44` **and** `frame.width >= 44` — the `ProfileButton`/`SearchButton` idiom (`.frame(width: 44, height: 44)`, "Fitts's Law minimum", `design-principles.md` §2), applied to controls that carry text rather than a glyph, so the 44 pt floor is a floor and not the size; (b) each label is non-empty, **contains the route's duration and distance as spoken words rather than the raw strings alone**, and the two labels differ; the Scenic control's `.success` label contains its `viaHoodName`; in each of the 6 disabled cases the label contains the §4.10 explanatory line — so a VoiceOver user learns *why* it is disabled, which sighted users get from the visible line. `isEnabled == false` is asserted alongside, never instead; (c) with the element count re-asserted **3**, each control's rendered height is **strictly greater** than at the default size, and **no label renders an ellipsis** — the durations, distances and `via <Hood>` text must wrap or grow, never truncate. `grep` for `.lineLimit` does **not** check this (T-038 §9 row 8(c-ii)'s finding): `.fixedSize(horizontal: false, vertical: true)` is what prevents it, and its presence is asserted in source as a supporting fact, not as the check; (d) both lines render in full at that size with non-empty frames, and the Go button's frame does not cover either (§9 row 5c's condition, re-run at the large size where overlap is likeliest). **All of (a)–(d) are rendered-output checks; any that cannot be run is BLOCKED, not passed** (standing rule (c)) | UI test | C5, C7, C8 |
...
| **C5** | **[iOS]** | `Detail/RouteControls.swift` — the five model states, the six disabled lines, durations and distances (§4.10). **Per A3:** 44×44 minimum frames in the `ProfileButton`/`SearchButton` idiom, an `accessibilityLabel` on each control that speaks its duration/distance and — when disabled — its §4.10 reason, and `.fixedSize(horizontal: false, vertical: true)` on every text run so nothing truncates at AX5 (§9 row 15). |
| **C6** | **[iOS]** | `Map/MapScreen.swift` — render `RouteLayer`, fit the camera once with a bottom inset **read from the rendered sheet frame at runtime — never a device constant** (§4.9, amendment A3; §9 row 12 greps for one), own `RouteMemoStore` as `@State` and inject it on the same `Group` as `placeCatalog`/`detailRouter`/`savedPlacesStore` (the call site whose comment explains why the sheet's content closure doesn't inherit environment values applied elsewhere), and call `clearAll()` from the existing `.onChange(of: scenePhase)` on `.background` (§4.7). |
| **C7** | **[iOS]** | `Detail/PlaceDetailModal.swift` + `Support/DirectionsService.swift` — host `RouteControls`, add the waypoint disclosure line, preserve T-033's Go behaviour and its `availableApps().isEmpty` branch verbatim (§4.8). **Per A3:** the disclosure line and the `via <Hood>` line must survive AX5 without truncation or being covered by the Go button (§9 rows 5c, 15d). |
| **C8** | **[iOS]** | Test pass: §9 rows 1–7 and 9–**15**. Scripted provider throughout; UI tests assert rendered frames, never `.exists` alone. Row 15 and row 12(b) both need a run at `UICTContentSizeCategoryAccessibilityExtraExtraExtraLarge`, not only at the default size. |
...
| **D17** | **The camera inset is measured from the rendered sheet, and `RouteControls` gets its own accessibility row** *(amendment A3, 2026-08-04)* | Both are the same failure in two places: a check written against one device's snapshot. The inset was justified by a real `qa` measurement (iPhone 17, `.medium`, y 415–866) — good evidence that an inset is *needed*, and the wrong thing to ship, because sheet height moves with Dynamic Type and size class while a constant does not, and the route lands under the sheet in exactly the configurations least likely to be reviewed. The accessibility gap is the same shape: `RouteControls` was the only interactive surface in this feature with no §9 row, and it is the one carrying text (durations, distances, `via <Hood>`) rather than a glyph, so it is the most likely to truncate at AX5 and the only one that needed to explain *why* it is disabled to a VoiceOver user. Per L-032, a normative statement without a row is a rule no gate reads — so both became rows rather than build-time advice. Raised by `ios-code-reviewer` at `trd-review` (`passenger-brain dd68110`, findings 2 and 3). |
...
**Outcome, 2026-08-04: all four AGREE.** `ios-developer` (`passenger-brain 8a006f7`), `ios-code-reviewer` (`dd68110`, five non-blocking build findings), `data-engineer` (`5bfee4d`, one authoring-detail ambiguity), `code-reviewer` (A1's migration path). `ios-developer`'s two pre-C3 findings are resolved in this document as amendments A1 and A2, and `ios-code-reviewer`'s findings **2 and 3** as amendment A3 (see the header) — all documentation-only, no re-review owed. Item 1 below is **resolved**; the rest stand.

**Still open from `ios-code-reviewer`'s five, deliberately not folded into A3** (its findings 1 and 4, both carried as build-time judgment calls for `ios-developer`, neither blocking): **(1)** §4.6's 2.5 s `resolveBudget` is enforced *between* candidate attempts, not within one, so a single slow — not throttled — `MKDirections` call has no forced cutoff and req 7's 3 s is a soft target; §9 row 7(c)'s BLOCKED-if-throttled condition names this honestly, and a per-leg timeout race in the provider is the fix if C3 finds it matters. **(4)** the one-time camera fit animation does not address Reduce Motion. Both were left as they are because they are behaviour changes, not documentation catch-ups; A3 covers only the two findings that were verification defects.

...
**Task:** T-037 · **Linear:** `PAS-28` · **Status:** v3 — **amended 2026-08-05 by `architect` (T-069/`PAS-65`), verification only.** §9 row 3's pass condition now checks that a sticker **depicts** its place type, not merely that every type resolves to *some* shape; §9 row 7 and **D12** are corrected to match, because `product` rejected D12's premise at T-037 acceptance (`passenger-brain 1d1167f`). **No architectural change** — no module, contract, data model, flow or build step is redesigned; §11's A1/C2/C5 keep their scope and gain nothing but a stricter check to pass. Prior: v2 — **amended 2026-08-04 by `product`, D6 only** (T-048/`PAS-35`: the Local threshold is confirmed at 2, decision #45). No architectural change: §1 row 3, §4.2, D6, §11's risk row and §12's routing lines are updated to stop describing a decided number as provisional. Everything else is `architect`'s v1, untouched.
**Owner:** architect · **Date:** 2026-08-03 (v1) · 2026-08-04 (v2 amendment, `product`) · 2026-08-05 (v3 amendment, `architect`)
**PRD:** [`passport.md`](./passport.md) (Draft v3)
...
| 3 | ~~The Local threshold is a number nobody has stated~~ **Decided 2026-08-04** | PRD Open questions ("cannot ship without them") | One app-wide constant, `LocalStatus.threshold`, **value 2 — confirmed, decision #45**, no longer `[ASSUMPTION]` and no longer flagged for Aviran. Not per-Hood, not a view constant | §4.2, **D6** |
...
| 9 | ~~Req 7's *"VoiceOver label naming the place and its type"* vs. T-042 §4.4's *"no user-facing surface renders `place_type`"*~~ **Not a conflict — premise rejected 2026-08-05** | found by reading the two documents against each other | The label names **what the sticker depicts**, from a client-side `StickerShape.spokenName` — never the raw `place_type`. **No longer a deviation:** `product` rejected D12's premise at T-037 acceptance; a depictive sticker's noun ("cutlery") satisfies both documents, and a geometry word ("triangle") satisfies neither | §4.7, **D12** |
...
enum LocalStatus {
    /// Confirmed 2026-08-04 — decision #45. Not provisional.
...
**Why 2 — confirmed 2026-08-04, decision #45 (`product`, T-048/`PAS-35`).** The value is unchanged; the *reasoning* is replaced, because this section previously disclaimed being a product judgement and a shipping number cannot rest on observability alone. **Local per Hood means you came back, not you completed the Hood** — one 20-minute dwell is a tourist having lunch; two distinct curated places in the same Hood, each earned by a separate dwell, is someone who returned on purpose, which is the weight *"Land like a tourist. Move like a local."* asks the word to carry. The range was fixed by the data before judgement entered: **≥2**, because at 1 the status is true on first contact and overall Local degrades into a checklist; **≤3**, the smallest curated place count across designated Hoods, above which Local is unreachable by construction (PRD req 4 can never pass; `places-dataset` req 7). Within [2, 3], **3 is rejected** — it makes Local identical to 100% completion of a Hood's catalogue and leaves zero margin between reachable and unreachable for a number that gates dataset validation. **[ASSUMPTION]** 2 holds for roughly 3–6 curated places per designated Hood; re-derive if a designated Hood leaves that band or the designated set changes (T-047), and re-derive as one constant — never per-Hood.

The former observability argument still holds as a *consequence* and is worth keeping: with T-036's fixture (two `been` places, both in `kerem-hateimanim`) and A2's three designated Hoods, a threshold of 2 puts `kerem-hateimanim` at Local, the other two at a plain zero state, and overall Local at not-reached — Local, partial-zero and overall-not-reached all reachable on a real device, with only overall-Local-reached left to unit tests (§9 row 4). `validate_dataset.py`'s skipped check is now concrete: every designated Hood holds ≥2 curated places (`PAS-6` item 12); all three pass today at 3 each.
...
**D12 — the sticker label names what the sticker depicts, not the `place_type` string.** PRD req 7 bullet 1 asks for *"a VoiceOver label naming the place and its type."* T-042 §4.4 states that no user-facing surface renders `place_type` and that its only user-visible consequence in V1 is a sticker's shape — and a VoiceOver label is a user-facing surface.

**Amended 2026-08-05 (T-069): v1 claimed these two "cannot both be satisfied literally" and asked `product` to confirm a deviation. `product` rejected that premise at T-037 acceptance** (`passenger-brain 1d1167f`): §4.4 forbids rendering the raw server vocabulary *string*, not naming in the app's own words what kind of place something is. The contradiction existed only because v1's shape vocabulary was abstract, so the only word available to speak was a geometry word — and *"Dr. Shakshuka, triangle sticker"* is what shipped and what was rejected (F2). Once the sticker depicts the type, one client-side noun satisfies both documents at once and no deviation remains.

The mechanism is unchanged and still correct: `StickerShape.spokenName` is a client-side word per case — the same idiom as `PlaceCategory.displayName`, documented as *"the only place a user-facing category string exists."* The word lives on the client enum and **not** in the registry file or in `place_types`, so T-042 §3.1's "no display string on the server, because a display string invites a surface to render it" stays intact. Because C2's totality test guarantees every registry key resolves to a `StickerShape` case, every sticker has a spoken word by construction. What changed is only what that word is allowed to be: a noun for a kind of place ("cutlery", "cup"), never a geometry word — enforced by §9 row 3(f), checked by §9 row 7(a).
...
### D6 — The Local threshold is one Swift constant, value 2 (confirmed)
Req 4's "read from configuration, not hardcoded per Hood" is satisfied by one declaration site. A server config table does not exist and is not built for a number that changes a handful of times before launch. **Amended 2026-08-04 (`product`, T-048/`PAS-35`, decision #45): the value is confirmed at 2 and is no longer `[ASSUMPTION]` or flagged for Aviran.** The original reasoning — observability against shipped data, explicitly disclaimed as a product judgement — is superseded by one: Local means *came back*, not *completed the Hood*, inside a [2, 3] range the dataset fixes independently (§4.2). Per-Hood remains rejected; PRD req 4 forbids it by name. **One code follow-up, `ios-developer`'s (T-048 board row):** `LocalStatus.swift`'s doc-comment still reads "**[ASSUMPTION]** — provisional, Aviran's or `data-engineer`'s to set" and is now false. Comment only — the value does not change.
...
| **3** One sticker per Been place, off the existing signal; Visited earns none; Saved-alone earns none; revisit adds none; **shape depicts type**; every type has a shape; filed under the city page | (a) `PassportComposition.stickers` over a fixture with a Been place, a Visited place, a Saved-only place, a place in no source, and a visit id matching no `Place`; (b) the shipped `places-tel-aviv.json` decoded through `PlaceCatalog`; (c) C2's registry totality walk; (d) C12's bundle-coverage walk; (e) grep for a `city` field on `Place`/`Hood`; **(f) every `StickerShape` case name and every `spokenName` string, read as words; (g) each of the six shipped `place_type` values' sticker rendered at album size with the place name and every other label hidden** | (a) exactly one sticker, for the Been place; the Visited, Saved-only and unsourced places yield none; the unresolvable id is skipped without a crash; output is name-ascending and identical across runs; (b) all nine places decode with a non-empty `placeType` **and** T-036's `permanentlyClosed` together (§2.2); (c) every registry key maps to a non-`.generic` case whose `symbolName` resolves — **totality only. This clause is not req 3 bullet 5 and never was**: `cafe→circle, restaurant→triangle, bar→diamond` satisfies it in full and shipped, and was rejected at acceptance 2026-08-05 (F1, `passenger-brain 1d1167f`). (f) and (g) are the depiction check it was missing; (c) alone can never pass this row again; (d) every `place_type` in the places bundle is a registry key; (e) zero — exactly one header renders, from one constant (D11); **(f) zero geometry words.** Every `StickerShape` case name and every `spokenName` value is a common noun for a kind of place or for the thing that kind of place is recognised by (cup, cutlery, glass, basket, columns, monument). Pass = **zero** whole-word, case-insensitive hits over the closed list `circle, square, triangle, diamond, star, hexagon, pentagon, octagon, oval, rectangle, polygon, dot, blob, shape` across both the case names and the `spokenName` strings; one hit fails the build step, not `qa`. *Positive control, asserted before the search:* the enum exposes **≥6 non-`.generic` cases** and the registry **≥6 keys** — the search must never run over an empty set (L-032); **(g) the name-hidden depiction test** (`passport.md` req 3's pass condition, added at acceptance — this row exists to carry it, not to restate it loosely). For each of the six shipped values, a reviewer who has not read the registry answers *"what kind of place is this sticker for?"* from the glyph alone, and the answer is **recorded verbatim per shape**. Pass = each answer is that `place_type` or a plain synonym — `cafe` ← coffee/café; `restaurant` ← restaurant/food/eating; `bar` ← bar/pub/drinks; `market` ← market/stall/grocer; `museum` ← museum/gallery; `landmark` ← landmark/monument/sight. **Fail** on any geometry word, any "don't know", and any answer that names a *different* one of the six. Six shapes rendered and six answers recorded is asserted before any answer is graded; a shape that renders to an empty or clipped frame fails on that alone. Which specific glyph a type gets stays A1's and B1's to choose — this clause fixes the bar the choice has to clear, not the choice. **Not verified here: the 20-minute dwell threshold and the known-place guard — no detector exists.** Inherited from T-036 §9 row 3, re-run at Phase 2 against the real detector | (a)–(f) unit + review; **(g) manual, on rendered output** — `XCUIElement.exists`/`.label` cannot see a glyph's meaning, so no automated layer can stand in for it; if (g) cannot be run the row is **BLOCKED**, never passed | A1, C1, C2, C4, C12 |
...
| **7** Every sticker has a VoiceOver label naming the place and what it is; per-Hood status announced as text with count and Local state; every interactive target ≥44pt | (a) `PassportLabels` over the full shape × (Local, partial, zero) matrix; (b) the accessibility tree of the rendered screen; (c) the frame of every interactive element | (a) every string names the place **and a word for the kind of place the sticker depicts**, and every Hood string carries both numerals and the Local state — asserted against expected strings, not merely non-empty. **`"Dr. Shakshuka, cutlery sticker"` passes; `"Dr. Shakshuka, triangle sticker"` fails** (`passport.md` req 7's pass condition, added at acceptance). Row 3(f)'s zero-geometry-word list applies to every `spokenName` reaching this label, so a geometry word cannot survive to be spoken; (b) no sticker exposes an image-only element and no Hood row exposes a value-only element; (c) the ✕ and the drag handle are the **only** interactive elements (D10) and both are ≥44×44pt. **No deviation is outstanding here any more: D12's premise was rejected by `product` at T-037 acceptance 2026-08-05** (`passenger-brain 1d1167f`) — req 7 and T-042 §4.4 were never in conflict, and the label now names what the sticker depicts, which is the substance req 7 asked for. Speaking the raw `place_type` string stays forbidden | unit + UI test | C5, C8, C13 |
...
| ~~The Local threshold ships as an invented number and nobody notices it was never ratified~~ **Closed 2026-08-04** | Was: one constant, one site, tagged **[ASSUMPTION]**, named in this TRD's review routing. **Ratified by decision #45** (`product`, T-048/`PAS-35`) with the reasoning replaced by a product argument. The residual risk is now a different one and is written into #45 as a review trigger: 2-of-3 and 2-of-15 do not mean the same thing, so the number needs re-deriving when the curated catalogue per designated Hood grows |
...
| # | Step | Tag |
|---|---|---|
| **A1** | Author `Passenger/Resources/place-types-tel-aviv.json` — the `place_type` → `sticker_shape` mapping for exactly the six values in the shipped places bundle (`bar`, `cafe`, `landmark`, `market`, `museum`, `restaurant`), with the PROVISIONAL `_note` naming T-042 B1 as the ratifying owner (§3.4). **Not invented by whoever writes the Swift.** **The pairing must depict, not merely map** (§9 row 3(f)/(g), amended 2026-08-05; `places-dataset.md` req 3's matching bullet): each value pairs with a glyph a person names as that kind of place. Still provisional under B1 — B1 now ratifies against that criterion | **[Algo/Data]** |
...
| C2 | `StickerShape` (closed enum, `.generic` fallback, `symbolName`, `spokenName`) + `PlaceTypeRegistry` — bundled, synchronous `static let`, **not** wired into `PlaceCatalog.load()` or any `.task` (§3.3, §3.4, D3, D12). **Totality test:** every key in the shipped registry maps to a non-`.generic` case whose `symbolName` resolves to a real symbol. Missing/corrupt registry → empty map, never a crash. **Every case is a depiction, not a geometry** (§9 row 3(f)/(g), amended 2026-08-05): case names and `spokenName` values are nouns for kinds of place — the zero-geometry-word assertion, with its positive control, is part of this step and fails the build, not `qa` | **[iOS]** |
...
- **`product` and Aviran** — the three PRD findings carried forward untouched (`place_type` user-facing or not; nothing rewards answering local-QA; device-local loss on reinstall), plus ~~**D6's threshold value**~~ (**closed 2026-08-04, decision #45 — 2, confirmed**), **D7's designated set** (still open, T-047), **D8** (no in-the-moment surfacing, `ux-flows.md` Q13 left open), **D12** (req 7's literal wording), and §4.4's name-ascending album order.
- **`chief-of-staff`, for `BOARD.md` rows this TRD cannot create** — **all three now have rows (filed 2026-08-03) and two are resolved:** the shared dwell/geofence detector is T-046/`PAS-33` (scope gate cleared 2026-08-04, decision #46 — V1 scope and launch-blocking, not parked Phase 2; two founder questions open before its PRD); `hoods.designated_for_progression` is T-047, still unowned; the Local threshold is T-048/`PAS-35`, **decided 2026-08-04, decision #45**. Original text follows: the shared dwell/geofence detector is still ownerless (named by T-035 and T-036 and now T-037, no row); `hoods.designated_for_progression` has no owner for its real values; the Local threshold has no owner. Per `CLAUDE.md` rule 5, each needs a row with an owner rather than another worklog line saying "flagged for whoever owns it."
...
| Date | Decision | Why |
|---|---|---|
| 2026-08-03 | TRD v1 written from `passport.md` Draft v2 (`architect`, T-037/`PAS-28`) | — |
| 2026-08-04 | **v2, D6 only** — the Local threshold is confirmed at 2, no longer provisional (`product`, T-048/`PAS-35`, decision #45) | A number the TRD had tagged **[ASSUMPTION]** was ratified; §1 row 3, §4.2, D6 and the routing lines were describing a decided value as open |
| 2026-08-05 | **v3 — §9 row 3's pass condition now requires depiction, not totality** (`architect`, T-069/`PAS-65`). (f) zero geometry words across `StickerShape` case names and `spokenName` values, with a positive control so it cannot pass over an empty set; (g) the name-hidden reviewer test on rendered glyphs, answers recorded verbatim, **manual** layer, **BLOCKED** if unrunnable. §9 row 7(a) and **D12** corrected to match; §11 A1 and C2 now name the bar. No architectural change | Root cause of T-037's acceptance **REJECT** (`product`, `passenger-brain 1d1167f`, L-009). The v1/v2 pass condition — *"every registry key maps to a non-`.generic` case whose `symbolName` resolves"* — checked totality and symbol resolution only. `cafe→circle, restaurant→triangle, bar→diamond` satisfies it perfectly and is a bijection, not a depiction, so the build shipped abstract geometry through `code-review` and a 485/485 `qa` PASS with nothing anywhere to fail it on. `qa`'s PASS was correct against the check it was handed. The criterion is taken verbatim from the bullets `product` added to `passport.md` req 3/req 7 and `places-dataset.md` req 3 in the same commit — not invented here |
| 2026-08-05 | **D12's premise rejected, not just its ruling** (same amendment) | D12 claimed req 7 and T-042 §4.4 "cannot both be satisfied literally" and asked `product` to confirm a deviation. §4.4 forbids rendering the raw `place_type` *string*; naming what a place is in the app's own words is a different act. The conflict existed only because the shape vocabulary was abstract, which left a geometry word as the only thing to speak. Leaving D12 standing would have sent the rebuild back to *"Dr. Shakshuka, triangle sticker"* — the exact string F2 rejected |

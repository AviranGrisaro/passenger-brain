...
**Author:** `architect`, 2026-07-31 · **Amended:** 2026-08-03 — **v2**, the T-049/`PAS-37` ruling (§2.2 **D6**, §3.3, §4.2, §4.5 **D7**, §5, §9 B3, §11)
**Status:** **v2.** `trd-review` cleared 3/3, approved as-is (`developer`, `data-engineer`, `code-reviewer`, 2026-07-31) plus a `security-auditor` PASS. `build`-ready but **HELD to Build Phase 2**, undispatched. v2 is a documentation amendment ordered by `product`; no contract in it is re-opened and no re-review is owed — see [§11](#11-decisions-log).
...
| Authored source — places | `database/data/places-tel-aviv.source.json` | new |
| Authored source — the shape registry | `database/data/place-types-tel-aviv.source.json` | new at v2, [§2.2](#22-not-built-here) D6 |
...
| `place-types-tel-aviv.json` — the **bundled** `place_type` → `sticker_shape` registry the client reads. Hand-authored as a Phase-1 placeholder by T-037 A1; **emitted by this task's `export_places.py` ([§9](#9-build-breakdown) B3) from Build Phase 2 onward**, see D6 below | **T-042 (this task), B3** — placeholder authored by T-037 A1 |
| Sticker-shape totality test — every `sticker_shape` in the shipped `place-types-tel-aviv.json` registry resolves to a real asset | T-037 (Passport) |
...
**Decision D6 — `export_places.py` ([§9](#9-build-breakdown) B3) also emits the bundled registry `place-types-tel-aviv.json`, from the same validated source that produces the `place_types` seed rows. Added at v2 (T-049/`PAS-37`).** D7 makes the registry a bundled artifact rather than a fetch, and a bundled artifact with no named emitter is exactly the gap that produces a hand-edited "generated" file (L-024). Today the shipped `passenger-code/Passenger/Resources/place-types-tel-aviv.json` (T-037 step A1, `passenger-code cc2b008`) carries the header *"Regenerate/replace, do not extend by hand"* and **nothing in this workspace could regenerate it** — `database/scripts/` has no `export_places.py` yet, and `database/data/places-tel-aviv.source.json` does not exist. This decision closes that:

- **One source, two artifacts, twice over.** B3 already writes the DB seed and `places-tel-aviv.json` from one validated source ([§5](#5-flow) step 3, D4). The registry is the same rule applied to the second table: the authored `place_type` → `sticker_shape` vocabulary (B1, Aviran-ratified) yields **both** the `place_types` insert statements **and** `place-types-tel-aviv.json`, in one run. The bundle and the table cannot disagree, because neither is hand-written. Same relationship `build_hoods.py` has to its two artifacts (`hood-dataset/TRD.md` §3.3/§3.5, §11 B2).
- **The registry's authored home is `database/data/place-types-tel-aviv.source.json`** — a `{ "<place_type>": "<sticker_shape>" }` map, sibling to the places source, the only hand-edited file on this path. B2's validator already fails a `place_type` with an empty `sticker_shape` ([§5](#5-flow) step 2); it also fails a `place_type` used by any place but absent from this map, which is the source-side half of T-037's C12 invariant.
- **The emitted file must keep the shape T-037 already ships and decodes** — `schemaVersion`, `_note`, `types` — with the `_note` rewritten by the generator from `PROVISIONAL …` to a generated-artifact banner naming `export_places.py`. Changing the shape would be a client change, and this task has no `[iOS]` step.
- **T-037's A1 file is a hand-authored Build-Phase-1 placeholder, and it is legitimate.** It shipped before any generator existed, covering exactly the six `place_type` values in the shipped places bundle, explicitly labelled `PROVISIONAL` against B1. **L-024's hand-edit ban attaches to this file from the moment B3 ships, not before** — until then a hand-edit is the only supported path and the `_note`'s own instruction is unfollowable. From B3 forward, the file is generator output: edit `place-types-tel-aviv.source.json` and re-run, never the bundle. This is the same bounded exception, and the same retirement, that `hood-dataset/TRD.md` §3.5 grants its C2a hand-edit and retires at C3.
- **The retirement is a build step, not a note.** B3's Build-Phase-2 run overwrites T-037's A1 file; [§9](#9-build-breakdown) B3 states it, so the placeholder has a named end rather than an assumed one.
...
**`place_types_public_read` stays exactly as written, and the V1 client is not its reader** (added at v2, [§4.5](#45-fetch-contract) D7). The registry reaches the app bundled, not fetched, so nothing in V1 selects from `place_types` over the wire. The policy is kept anyway: it is one line in an unapplied migration, it keeps both tables under the same public-read shape a reader can check at a glance, and dropping it would mean editing a migration — or writing a second one — to remove a policy that grants read access to a six-row mapping of slugs to shape names, carrying no user data and no display string ([§3.1](#31-publicplace_types)). Removing it churns a migration for no gain. Its readers are out-of-band: the seed's own verification, `psql`, and anything later that legitimately needs the registry server-side.
...
- **Client half (built by T-037, named in [§2.2](#22-not-built-here)):** a `StickerShape` Swift enum over the known shape keys, plus a test asserting **every** `sticker_shape` present in the shipped **`place-types-tel-aviv.json`** registry resolves to a real asset. The test is the falsifiable pass condition; the enum alone is not, because the key arrives as a `String`. The filename is the one that actually shipped (`passenger-code cc2b008`) and matches the `<thing>-tel-aviv.json` convention `hoods-tel-aviv.json` / `places-tel-aviv.json` already set — v2 corrected it from an earlier `place-types.json`, which never existed. The registry is a **bundled build-time artifact**, so this test is a build gate rather than a runtime hope ([§4.5](#45-fetch-contract) D7); it is emitted by B3 from Build Phase 2 onward ([§2.2](#22-not-built-here) D6).
...
**One request, unchanged in shape.** No pagination. No query parameter carries anything user-specific. `place_type` stays inside the embedded `places(…)` select above: it is **per-row data** and must arrive live at Build Phase 2, exactly like `category` or `keywords`. Only the *registry* — the `id` → `sticker_shape` mapping — is bundled.

**Decision D7 — the `place_type` → `sticker_shape` registry is bundled in the app, never fetched. Permanent for V1. Added at v2 (T-049/`PAS-37`).** An earlier version of this section specified a second call, `GET /rest/v1/place_types?select=id,sticker_shape`, "fetched once per session alongside the places load." **That contract is withdrawn.** The registry reaches the client as `place-types-tel-aviv.json` in the app bundle; the design, its reasoning and the client-side loader all live in [`passport/TRD.md`](../passport/TRD.md) **§3.4 / D4**, which this document defers to rather than restating. Three reasons this document is the one that moves:

1. **Scope.** [§9](#9-build-breakdown) states *"No `[iOS]` step exists in this task"* and [§2.2](#22-not-built-here) assigns the client half of C-STICKER-1 to T-037 by name. "Fetched once per session" was a client-runtime claim in a document that specs no client behaviour.
2. **Self-consistency.** [§4.2](#42-c-sticker-1--place_type--sticker-shape) already defines this task's own falsifiable pass condition against *the shipped export* — a build-time artifact. A runtime GET three sections later contradicted it, and §4.2's check is only checkable against a bundled file.
3. **Nothing read it.** A `sticker_shape` key is undrawable unless the binary ships a matching asset, and an unknown key renders `.generic` however it arrived — so the request bought an undrawable key plus a cache field and a skew class. This task's own pipeline runs authored source → validator → export → DB seed ([§5](#5-flow)), so the database is **downstream** of the mapping, never upstream of it. No consumer of that endpoint exists anywhere in the workspace.

The `place_types` **table** and its `place_types_public_read` policy are unaffected — the FK from `places.place_type` is what makes req 3's closed enumeration structural ([§3.1](#31-publicplace_types)), and that is a database-side guarantee, not a transport. [§3.3](#33-rls--the-established-public-read-shape-not-a-new-one) records that V1 has no client reader for it. The bundled artifact's emitter is [§2.2](#22-not-built-here) D6.

**Ruling, not preference:** `product`, 2026-08-03, Linear `PAS-37` / T-049, after both TRDs' full text was read against each other. If it is ever reversed, the change here is one line in the block above; on the client it is *"one loader conformance and one `PlacesAPI` method"* (`passport/TRD.md` §3.4).
...
3. `export_places.py` writes **four** artifacts from the same validated sources in one run — two per table, one DB-side and one bundle-side:
   - from `places-tel-aviv.source.json`: the `places` seed statements, and `passenger-code/Passenger/Resources/places-tel-aviv.json` (T-033's bundled seed floor, extended per [§2.2](#22-not-built-here));
   - from `place-types-tel-aviv.source.json`: the `place_types` seed statements, and `passenger-code/Passenger/Resources/place-types-tel-aviv.json` (the bundled registry T-037 reads — [§2.2](#22-not-built-here) D6, [§4.5](#45-fetch-contract) D7).

   Same one-source-two-artifacts rule `hood-dataset` req 6 sets for Hoods; within each pair the two can never disagree, because neither is hand-edited.
...
| # | Step | Tag |
|---|---|---|
| B1 | Propose the initial `place_type` enumeration and its `sticker_shape` mapping, for Aviran's ratification. Not invented by whoever writes A1. **The ratified result is checked in as `database/data/place-types-tel-aviv.source.json`** ([§2.2](#22-not-built-here) D6) — the authored home of the vocabulary, and the input B3 emits both the `place_types` seed rows and the bundled registry from. It supersedes T-037 A1's provisional six-value file, which named this step as its ratifying owner | **[Algo/Data]** |
...
| B3 | `export_places.py` — validated sources → **four artifacts in one run** per [§5](#5-flow) step 3: the `places` seed + `Resources/places-tel-aviv.json`, and the `place_types` seed + `Resources/place-types-tel-aviv.json` ([§2.2](#22-not-built-here) D6). Fails the export on any validator failure, so no artifact is written at all. **This run overwrites T-037 A1's hand-authored placeholder registry and retires it** — from this step forward that file is generator output and L-024's hand-edit ban applies to it; edit `place-types-tel-aviv.source.json` and re-run instead. Preserve the file's shipped shape (`schemaVersion` / `_note` / `types`), rewriting `_note` from `PROVISIONAL …` to a generated-artifact banner naming this script, since changing the shape would be a client change and this task has no `[iOS]` step. Deterministic output (keys sorted, no wall-clock stamp — `build_hoods.py`'s `generatedAt` is the counter-example, it makes every regeneration diff dirty; `hood-dataset/TRD.md` §3.5) | **[Algo/Data]** |
...
5. **The bundled registry has an emitter on paper only until B3 ships** (added at v2). D6 names `export_places.py` as the emitter of `place-types-tel-aviv.json`, but B3 is Build-Phase-2 and this task is HELD, so the file living in `passenger-code` today is T-037 A1's hand-authored placeholder with a *"do not extend by hand"* header nobody can currently obey. Until B3 lands, editing that file by hand is the **only** supported path and is not an L-024 violation; from B3 forward it is. Anyone adding a `place_type` in that window edits the bundle *and* must carry the same value into B1's source when it lands — which is the drift D6 exists to end, surfaced rather than assumed away.

---

## 11. Decisions log

| Date | Version | Decision | Why / source |
|---|---|---|---|
| 2026-07-31 | v1 | D1–D5 as written above; `trd-review` cleared 3/3 approved-as-is (`developer`, `data-engineer`, `code-reviewer`) plus `security-auditor` PASS | Original document |
| 2026-08-03 | **v2** | **The `place_type` → `sticker_shape` registry is bundled, never fetched, permanently in V1. `GET /rest/v1/place_types?select=id,sticker_shape` and its "fetched once per session" sentence are struck from [§4.5](#45-fetch-contract) (new **D7**). `place_type` itself stays in the embedded `places(…)` select — per-row data, live at Phase 2.** | `product`'s ruling on **T-049 / Linear `PAS-37`**, 2026-08-03 (`passenger-brain 3a0824a`), reconciling this document with [`passport/TRD.md`](../passport/TRD.md) **§3.4 / D4**. Raised independently by `data-engineer` and `code-reviewer` at T-037's `trd-review`. Grounds in [§4.5](#45-fetch-contract): this document's own [§9](#9-build-breakdown) says it specs no client behaviour; its own [§4.2](#42-c-sticker-1--place_type--sticker-shape) already assumed a bundled export; no consumer of the endpoint exists |
| 2026-08-03 | **v2** | **`place_types_public_read` is kept exactly as written, with a stated note that the V1 client is not its reader** ([§3.3](#33-rls--the-established-public-read-shape-not-a-new-one)) | Removing a policy from an unapplied migration churns a migration for no gain; the table's FK guarantee is unaffected by the transport decision |
| 2026-08-03 | **v2** | **`place-types.json` → `place-types-tel-aviv.json`** throughout ([§4.2](#42-c-sticker-1--place_type--sticker-shape)) | Corrected against what actually shipped — `passenger-code cc2b008`, `Passenger/Resources/place-types-tel-aviv.json` — and the `<thing>-tel-aviv.json` convention T-040/T-042 already use. `place-types.json` never existed |
| 2026-08-03 | **v2** | **`export_places.py` (B3) is named the emitter of the bundled registry, from a new authored source `database/data/place-types-tel-aviv.source.json`; T-037 A1's file is a Phase-1 placeholder the generator supersedes, with L-024's hand-edit ban attaching from B3 forward** (new **D6**, [§2](#2-architecture), [§5](#5-flow) step 3, [§9](#9-build-breakdown) B1/B3, [§10](#10-left-for-the-board) item 5) | Second finding in the same `product` ruling: the shipped file says *"Regenerate/replace, do not extend by hand"* and **no generator existed** — verified, `database/scripts/` holds no `export_places.py` and no places source file. A bundled artifact with no named emitter is L-024's failure mode one build phase away. Mechanism is `architect`'s call; the shape follows D4's existing one-source-two-artifacts rule |

**No re-review is owed for v2.** Every change is a documentation reconciliation ordered by `product` after both TRDs were read in full: no requirement is added, cut or rescoped; no schema, migration, constraint or RLS policy changes a character; D1–D5 are untouched. The one contract that moves — §4.5's withdrawn second GET — is withdrawn *toward* the design the client is already being built against (`passenger-code cc2b008`), so nothing shipped or in flight moves either. D6 adds a named emitter and an authored source file to an existing `[Algo/Data]` step that is HELD at Build Phase 2, so it is the first reader of B3 that will act on it, not a re-plan of work in progress. `data-engineer` and `code-reviewer` see both at their Build-Phase-2 dispatch.

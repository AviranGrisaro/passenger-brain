...
| Piece | Where | New? |
|---|---|---|
| `public.places` | `database/migrations/NNN_places.sql` | new |
...
**Migration number is a naming detail, not a correctness one.** Nothing in this migration's SQL references `003`'s contents — it needs only `public.hoods` from `001`. Whoever writes it takes the next free number (expected `004`, behind T-040's `003`); if T-040 lands second, `003` is fine. The file is written order-independently regardless ([§7.2](#72-migration-safety)), so a build-order accident cannot silently ship a half-defined table.
...
|---|---|
| `places-tel-aviv.json` bundled fixture gains `place_type`, `keywords`, `permanently_closed`, `is_tourist_trap` | T-033 (owns the export, its step B2) |
| `Place` Swift model gains the four fields; `PlaceCategory` unchanged | T-033 |
...
| Closed-state **refresh** pipeline (cadence, owner, mechanism) | **nobody — open, see [§8](#8-risks--alternatives)** |
...
**Boundary case, pinned:** a place whose coordinates fall exactly on a shared Hood edge is resolved by the ray-cast's own deterministic result, not by file order (the failure mode `HoodHitTester.swift` has today). Because clause 2 fails on two containing Hoods and `hood-dataset` req 2 forbids overlap, at most one can ever claim it.
...
| **Closed-state refresh has no owner and no cadence.** `places-been-saved` req 4 needs a place that closes *after* being saved to show the badge next render; a one-time author-time read cannot deliver that. | **Explicitly out of scope and explicitly unowned** — the PRD fences the update pipeline out, and this TRD does not smuggle it back in. `closed_checked_at` is the mitigation that ships: staleness becomes visible instead of assumed. **Escalation, not a build detail** — flagged for `chief-of-staff` in [§10](#10-left-for-the-board). |
...
2. **Closed-state refresh remains unowned** — the PRD fences it out, this TRD does not adopt it, and `places-been-saved` req 4 depends on it. It has now been flagged by `product`, by T-036's PRD, and by this TRD without ever acquiring an owner.

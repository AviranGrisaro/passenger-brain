...
And the reason all three exist: **a requirement is verified at the layer the user perceives it, never at the value handed to that layer, and never at a value that layer no longer reads.** `isDimmed == true` is an input to rendering; `fillColor` is now a computation nothing renders; the check belongs on what rendered.

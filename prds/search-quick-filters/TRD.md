...
**Status:** **v3**, 2026-08-04 (v2 2026-08-03, v1 2026-08-03) — verification amendment only. No contract, decision or build step is re-opened; **no `trd-review` re-run is owed.**
**Author:** `architect` · **Amended:** 2026-08-04 — T-038/`PAS-55`: [§9](#9-verification--one-row-per-p0-requirement) row 4 (a)/(b)/(f) retargeted from `HoodLayer.fillColor` to `borderColor`; [§4.10](#410-the-dim) brought in line with the border-only render; new build step **C15**; new decision **D15** · **Amended:** 2026-08-03 — T-053/`PAS-43`: [§9](#9-verification--one-row-per-p0-requirement) rows 2, 3, 4, 8 and the §9 preamble; [§4.10](#410-the-dim) brought in line with the system as built; **D14** (`PAS-43` corrects an original mislabel of `PAS-41`, which is an unrelated `ios-developer` CI-workflow task — `chief-of-staff`, 2026-08-03)
...
**What changed at v3 (2026-08-04) — T-038/`PAS-55`, `qa`'s F-QA5.** A change outside this ticket's diff — **`PAS-49`** (`passenger-code c1b8bc3`, further fixed in `5e1f72f`, "HoodLayer polygons render border-only, no interior fill") — rewrote `HoodLayer.body` to draw `.foregroundStyle(.clear)` in every stroke branch. **`HoodLayer.fillColor` is no longer read by `body`**, so §9 row 4's pass conditions, and the whole of `HoodLayerFillDimTests`, were asserting a value the app does not render. The property still computes correctly, so the suite stayed green while proving nothing — a dead-assertion suite. The channel that renders now is **`borderColor`**, which does correctly carry `* dimOpacity` in both its branches (`HoodLayer.swift:165-169`, read directly) but has **zero test coverage anywhere** (`grep -rn borderColor PassengerTests/ PassengerUITests/` → 0 hits).

This is the **third** occurrence of L-009's shape on this one requirement (after T-046/`LOC-104` and T-053/`PAS-43`) and the second on row 4 specifically. v2 fixed *which property* the row asserts; v3 fixes it again because the property moved underneath the document. The lesson v3 adds, written into §9's standing rules: **a row must name the channel the view draws, and must fail if that channel loses its coverage** — naming a correct-but-unread property is the same defect as naming a flag instead of a render. Row 4 (a)/(b)/(f) retargeted, a coverage anchor added, **C15** added to retarget the tests, **D15** records it. **No behaviour changes and no `trd-review` re-run is owed** — see D15 for why `PAS-49`'s render change does not itself reopen this task.
...
`isDimmed` multiplies the layer's own opacity by `0.25`; when `false` the rendering is byte-identical to what ships today, so the non-search path cannot regress.

**"Every visible element" means the channel `HoodLayer.body` actually draws, and that channel changed on 2026-08-04.** Until `PAS-49` it was the polygon's heat **fill**, which dominated a 0.5pt stroke and a centroid capsule; a dim reaching only the stroke and the label was not a dim a user sees (PRD req 4 bullet 2). `PAS-49` (`passenger-code c1b8bc3`/`5e1f72f`) made a Hood **border-only** — `body` passes `.foregroundStyle(.clear)` in every branch, and the Hood's heat colour moved into **`borderColor`**, boosted past the fill's translucent steps (`min(HeatPalette.opacity(for: band) + 0.35, 1)`) because a thin stroke at fill-strength alpha is nearly invisible. `borderColor` multiplies by the same `dimOpacity`, so the requirement is still met — but by a different, thinner channel than the one this section was written around.

The dim is still a *numeric* composition rather than a view modifier, for the reason that has not changed: it makes "does the dim reach what renders" a unit-testable question about two `Double`s instead of a fact about a SwiftUI modifier chain no test can observe. What changed is **which** number. `fillColor` remains in the file and still composes `HeatPalette.fillOpacity(for:dimmedBy:) == opacity(for: band) * dimOpacity` correctly, but **it is not a render channel and must never be used as evidence about the render** (D15). §9 row 4 asserts `borderColor`.

A `nil` band keeps `fillColor == .clear`, and draws a neutral `.secondary` hairline border at `0.35 * dimOpacity`. That is a real difference from the fill era: a `nil`-band Hood's *fill* had nothing to multiply, so it dimmed by zero, while its *border* dims like every other Hood's.
...
**Narrowed at v3, not resolved — still `product`'s.** Under the border-only render the matched `nil`-band Hood keeps a **full-strength neutral outline** while the other 23 outlines drop to 0.25, so there *is* a positive mark on the match, not only the absence of dimming elsewhere. The limit that survives is weaker and worth restating precisely: the mark is **uncoloured**, not absent. `product` should confirm that reading rather than inherit the v2 wording.
...
**A third rule, added at v3 after row 4(a) passed a second time while asserting a property the view had stopped reading (T-038/`PAS-55`, `qa`'s F-QA5).**

- **A row names the channel the view draws, and carries a coverage anchor that fails when that channel loses its tests.** Naming a property that computes correctly but is never rendered is the same defect as naming a flag instead of a render — the suite stays green and proves nothing. Every row in this table that asserts a colour or an opacity runs a **positive coverage grep first** (`grep -rn "<the named property>" PassengerTests/ PassengerUITests/` → **> 0 hits**); zero hits means the row is **unrun**, not passed, regardless of what the suite reports. And when a row names a property that is `private` in the source, making it internal — with the doc-comment rationale saying why — is part of the step that implements the row, not a build-time judgment call.

**A fourth rule, ratified workspace-wide 2026-08-07 (`product` at T-077/`PAS-51` acceptance, `architect` ratification; canonical text now in `architect.md` §9).** It is lifted in rather than cross-referenced because `qa` reads this table on its own.

- **A check sequenced behind a known-failing sibling is unrun, not passed.** Under `continueAfterFailure = false` — set by eight of this repo's ten `PassengerUITests` classes — and equally under `try #require`, `XCTUnwrap`, or an early `return` inside a shared assertion helper, the *first* failing assertion aborts every assertion after it in that method, while `xcodebuild` still reports **one** failure line for the whole method. A gate that counts failure lines reads that silence as a pass. **So no check in this table may share a test method with an assertion that is known to fail and is owned by another ticket** — move the known failure into its own method, where it stays visibly red and tracked while its siblings still execute. Any report on this table states, per sub-check, *that it executed*; a sub-check whose execution cannot be shown in the result bundle is **unrun**, exactly like a BLOCKED row. **`XCTExpectFailure` is not the fix** — it turns the method green and buries the tracked gap. **Audit of this table against the rule, same pass:** its UI-test rows are discharged in `SearchAccessibilityTests` and the `PassengerUITests` search suites, none of which currently carries a disclosed known-failing assertion, so no row moves. The rule binds any future one.

And the reason all four exist: **a requirement is verified at the layer the user perceives it, never at the value handed to that layer, never at a value that layer no longer reads — and never on the assumption that a check ran because nothing reported it failing.** `isDimmed == true` is an input to rendering; `fillColor` is now a computation nothing renders; a sub-check after an aborting assertion is a check that never happened. The check belongs on what rendered, and the report belongs on what executed.
...
| **4** | Search filters the map at the selected hour; no second view of the data | (a) **the map's rendered Hood borders** (`HoodLayer.borderColor` — **retargeted at v3 from `fillColor`**, which `body` has not read since `PAS-49`), at a place-only query matching one place in one Hood whose band is **not** `nil` at the selected hour, run at **both** `.cityWide` (span 0.14, the cold-open camera) and `.close` (span < 0.06); (b) the **same rendered borders** after a result is selected and after the sheet is dismissed; (c) fetch surface of the feature; (d) the call made on a place-result tap vs a pin tap; (e) hour surface of the feature; (f) a result whose Hood has `band == nil` at the selected hour; (g) `SearchResultRow`'s content | (a) **at each of the two tiers, and stated as counts before any ∀:** all 24 `HoodLayer`s are constructed; **exactly one** is undimmed and it is the matched place's own Hood; that Hood's **resolved border alpha** (`HoodLayer.borderColor` → `UIColor(_:).cgColor.alpha`) is **unchanged** from its no-search value, and **every one of the other 23** resolves to an alpha **strictly less** than its own no-search value at the same band, each dimmed value equal to `0.25 ×` its own undimmed value within a stated tolerance — the ratio, not just the inequality, so a change that clamps or floors the multiplier is caught rather than passed. **The assertion is on the border channel — the only channel `HoodLayer.body` draws (`PAS-49`) — not on `fillColor` (dead code the view never reads), not on `isDimmed`, not on the centroid label.** Three conditions on the fixture, each of which silently invalidates the run if unmet: **(i)** `isHovered == false` on all 24 layers, or `hoverGlow`'s 1.4 multiplier is a second uncontrolled input to the same alpha; **(ii)** at least one Hood at each `HeatBand`, since `borderColor` clamps at `min(bandStrength × hoverGlow, 1)` and a single-band fixture cannot tell a correct multiply from a clamped one; **(iii) the coverage anchor, run before the assertions** — `grep -rn "borderColor" PassengerTests/ PassengerUITests/` → **> 0 hits** (it was **0** on 2026-08-04, which is what made this row unrun rather than passing), plus the standing positive control that the grep path list is right. If `borderColor` is still `private` at run time the row is **BLOCKED, not passed** — C15 makes it internal with the same rationale `fillColor` already carries in its doc comment. `Color.secondary` conversion is the one known fragility: if `UIColor(.secondary.opacity(x))` does not resolve to a comparable alpha in a unit test, assert the banded Hoods there and record the `nil`-band Hoods' borders as checked **manually/rendered or BLOCKED**, never as passed by omission. **At `.cityWide` the `PlaceLayer` count is asserted to be `0`** — that is the zoom tier this row's v1 condition was vacuously true at, so it is now the tier the check is *required* to cover, and the emphasis must be carried entirely by Hood fills there. Sanity-anchor the fixture first: the matched Hood's band is non-`nil`, otherwise the run is **BLOCKED, not passed** (the `nil`-band limit is named in §4.10 and flagged for `product`); (b) the previously-dimmed 23 Hoods' resolved **border** alphas are **back at their no-search values** in both cases — `emphasis == nil` is checked too, but as the cause, never as the evidence; (c) `grep -rn "URLSession\|PlacesAPI\|DensityAPI\|fetch" Passenger/Search Passenger/SearchSheet` → **0 hits**; (d) both are `router.openPlace(p)` with an identical `Place` — assert `router.place == p`; (e) `grep -rn "selectedHour" Passenger/Search Passenger/SearchSheet` → **0 hits** (the hour is session-scoped per T-032 §3.1, so "never an hour cached from a past session" holds structurally); (f) **retargeted at v3.** The rendered condition: `borderColor` for that Hood carries **no `HeatPalette.hue` component** — it is `.secondary` at `0.35 × dimOpacity`, a visible neutral hairline, not nothing — and no alert and no error copy appear. **A `nil`-band Hood is not invisible and was never meant to be**, so a check expecting "nothing renders" would fail a correct implementation. `fillColor == .clear` is retained as a regression guard on a still-computed value, explicitly **not** as evidence about the render (D15); (g) `grep -rn "isTouristTrap\|tourist\|Flag" Passenger/SearchSheet` → **0 hits** | unit + UI test + manual (visual dim) | C10, C11, C8, **C15** |
...
| **C15** | **Added at v3 (T-038/`PAS-55`), the only step this amendment opens.** Retarget the dim coverage at the channel that renders: make `HoodLayer.borderColor` internal (doc-comment rationale mirroring `fillColor`'s existing one), and rewrite `HoodLayerFillDimTests` to assert `borderColor`'s resolved alpha per §9 row 4 (a)/(b)/(f) — the 0.25 ratio, all three bands, `isHovered == false`, the `nil`-band neutral hairline. The existing `fillColor` assertions **stay** as a cheap regression guard on a still-computed value, but no longer carry the row. Rename the suite if `HoodLayerFillDim` reads as false advertising once it asserts borders. **No production behaviour changes in this step** — it is coverage, not a fix (D15) | **[iOS]** |
...
|---|---|
| 4(a) — the dim must reach the **heat fill** — **superseded at v3, see D15.** True as written on 2026-08-03 against `d537ca5`; `PAS-49` removed the fill from `body` the next day, so this row now describes a computation nothing renders | `HoodLayer.fillColor` = `HeatPalette.hue.opacity(HeatPalette.fillOpacity(for: band, dimmedBy: dimOpacity))`, and `fillOpacity` is `opacity(for: band) * dimOpacity`. The dim reaches the fill as a number. `HoodLayerFillDimTests` + `HeatPaletteTests` assert the resolved alpha, which is what the v2 row asked for — **and what the v3 row no longer asks for** |
...
### D15 — The dim is verified on `borderColor`, because that is what `HoodLayer` draws; `fillColor` is kept but demoted

**Date:** 2026-08-04 · **Task:** T-038 / `PAS-55` (`qa`'s F-QA5, raised in the T-038/`PAS-29` QA pass) · **Scope:** §9 row 4 (a)/(b)/(f), §4.10, C15. **Documentation and test coverage only — no production behaviour changes, and `build` is not reopened.**

**What happened.** `PAS-49` (`passenger-code c1b8bc3`/`5e1f72f`, a design fix landed by a concurrent ticket after T-038's round-9 review) made a Hood render as a border alone: `HoodLayer.body` now passes `.foregroundStyle(.clear)` in every stroke branch, and the heat colour moved into `borderColor`. `fillColor` still computes, is still correct, and is read by nothing. `HoodLayerFillDimTests` — the whole of this requirement's automated coverage — asserts it, so the suite is green and proves nothing about the render. `PAS-49`'s own commit message discloses this; fixing it was neither that ticket's scope nor T-038's build, and it fell into the gap between them.

**The decision.** §9 row 4 asserts **`borderColor`**. `fillColor`'s assertions are kept as a cheap guard on a live computation but no longer carry the row, and §4.10 says outright that it is not evidence about the render. `borderColor` becomes internal so a test can read it, the same trade `fillColor` already made and documented.

**Why not delete `fillColor`.** Tempting, and it would make this class of error impossible on this property. Rejected here because deletion is a production change to a shipped file, on a task at `qa`, to fix a *verification* defect — the two should not be bundled. It is worth doing, and it belongs to whoever owns `HoodLayer`'s render, with the design spec question below settled first. Named rather than quietly left.

**Why this does not reopen `trd-review` or `build`.** The requirement, the contracts and every build step are unchanged; `PAS-49` altered how a Hood is drawn, not what search does to it, and the dim still reaches the rendered channel by the same `dimOpacity` multiply. What changed is which number a test must read. Same treatment as D14, and the same reason.

**The general lesson, since this is the third occurrence (L-009: T-046/`LOC-104`, T-053/`PAS-43`, now this).** The first two were *wrong-layer* checks — a flag instead of a render, a ∀ over an empty set. This one is different and worse: the check was written at the right layer and became wrong when the layer moved. A verification row is a claim about the code, and it decays. §9's new third standing rule is the guard: name the drawn channel, and anchor every colour/opacity row on a positive coverage grep so that losing the channel's tests fails the row instead of silently passing it.

**Left open, not this decision's to make — and the reason `fillColor`'s deletion is not scheduled here.** `design/map-rendering-spec.md` §2 still states that "heat is always the area/fill treatment," which the shipped border-only render now contradicts, and `PAS-49`'s change is recorded in no PRD, TRD or design doc in `passenger-brain`. Owner: `designer` (that file's owner), with `product` if it turns out to be a scope question rather than a documentation one. **[ASSUMPTION]** that `PAS-49` was an intended design fix and not a regression — its commit message reads as deliberate ("Design fix (2026-08-04): a Hood is marked by its border alone"), and this amendment follows the shipped code on that basis; if it is reverted, §9 row 4 reverts with it.

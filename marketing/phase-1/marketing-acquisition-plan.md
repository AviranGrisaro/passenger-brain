...
**Copy note (2026-07-30, decision #36):** "tourist trap" replaced with **"tourist-heavy spot"** everywhere this plan speaks to a real person — Aviran flagged at the 2026-07-29 founders meeting that the old phrasing risks alienating/angering place owners. This is copy-only. It says nothing about whether the underlying map mechanic becomes a literal boolean "tourist-trap flag" replacing decision #18's three-way Local/Mix/Tourist tag — that question is decision #28, unconfirmed, tracked on Linear `PAS-6` (blocked on Aviran). The public phrase and the internal data model are separate questions; this plan only touches the former. Runner-up options considered: "touristy spot" (softer/more casual, but weaker on signaling "maybe skip it"), "mainstream pick" (drops the geographic/traveler framing that makes the pitch land), "crowd-magnet for visitors" (too colorful/informal for a first pass). Recommending "tourist-heavy spot": it's descriptive of foot traffic, not a verdict on the business, and it pairs cleanly with "real local spot" in the existing sentence.
...
- **"Tourist trap" → "tourist-heavy spot" (2026-07-30, decision #36) — copy-only, not a mechanic decision.** See the Positioning section note. Whether the map's underlying tag becomes a literal boolean flag (decision #28) is separate and unconfirmed (Linear `PAS-6`, blocked on Aviran); don't read this copy change as resolving that.
- **Stale-reference scan (this session, not fixed):** the Acquisition funnel's activation definition ("taps at least one **zone** or spot") predates decision #27 confirming **"Hoods"** as the product-facing term — decisions.md itself flags internal docs as not yet swept, so this is a should-fix, not new. This plan has no Passport/stamp-collection retention-hook copy yet (decisions #29/#31 pulled Passport into V1 after this plan was drafted) — worth a pass once product/designer settle the Passport-in-Profile-tab open question (strategy doc Open questions, item 4), not added here to avoid guessing at unconfirmed mechanics.

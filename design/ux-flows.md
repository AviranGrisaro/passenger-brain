...
**Status:** Locked, 2026-07-30 — Aviran confirmed all four PAS-6 items live the same day. **Routing preview and live events remain locked** from 2026-07-29 (§8a). **Confirmed 2026-07-30:** Hoods terminology (decision #27), Been-place Passport stickers (decision #29), Saved/Been distinction (decision #30, plus the closed-place badge, decision #38), Passport pulled forward into V1 with a real place in the nav model (decision #31), category rename (decision #33). **Resolved 2026-07-30, live confirmation (§9 Q18–Q21, decisions #37/#39/#40/#41 — Linear `PAS-6` closed):** tourist-trap boolean fully replaces the three-way Local/Mix/Tourist tag (§6 rewritten), "Profile" confirmed as the literal tab name (naming exception to the no-profiles gate, not a reversal of it), per-Hood Local status replaces the seven-tier ladder outright, quick filters stay sheet-internal (decision #25 stands). Still separately pending, unrelated to PAS-6: the search-sheet layout recommendation (§2.1, §9) and the place-detail button-hierarchy fix (§8a) — neither touched by this pass.
...
| Tag layer | **Tourist-trap flag, boolean (decision #37, confirmed 2026-07-30 — supersedes the old Local · Mix · Tourist tag)**: a Hood/place is flagged a tourist-heavy spot or it isn't, no graduated read | §3 never color alone; Tesler's Law | "Tourist-heavy or not" without asking | Primary · 0 taps. Rendering (§6, rewritten 2026-07-30) is a separate solution layer |
...
| Profile button (added 2026-07-30) | Chrome icon, the 3rd of the 3 side-by-side nav buttons — resolves the former "3rd button, TBD" question as option (c): opens the Profile tab housing **Passport** (decision #31). **Confirmed 2026-07-30 (decision #39):** "Profile" is the final name, a naming exception to the standing no-profiles gate, not a reversal of it | §2 Fitts's Law | Reliable, always-visible door to a private stats/progression screen | Primary · 0 taps (visible) / 1 tap (opens) |
...
**Addendum, 2026-08-02 (founder-direct, hilos).** All four persistent nav icons in the table above — Search, Heat, Profile, and the separate 4th Saved-places icon — render **icon-only: no visible text caption under or beside the glyph.** Aviran, live hilos `@chief` chat, verbatim: *"remove the name from the icons in the nav bar, show only icons"* (`PROGRESS.md`, 2026-08-02 FOUNDER-DIRECT STUB). This is a visual-only removal, not a loss of what the control is: each icon still carries a real `accessibilityLabel` naming it, same convention `NearMeButton`/`HoodButton` already ship in `passenger-code`. None of these four buttons has shipped Swift code yet — T-032's TRD builds the nav row container plus the Heat button only (its own D1); Search (T-038) and Profile (T-037) add their own buttons to the same container later, neither with a TRD yet. Recorded here, centrally, so whichever build order lands first builds icon-only from the start rather than shipping a caption and stripping it later.

~~Category chips~~ — **moved to Secondary, inside search (decision #25)**, overriding my own earlier flagged call. Not re-argued here. **Categories renamed 2026-07-30 (decision #33):** "Food & drinks" → **"Eat & Drink"**; "Things to do" unchanged. The founders-meeting brief calls these **"quick filters"** — **confirmed 2026-07-30 (decision #41): sheet-internal, decision #25 stands.** "Quick filters" was shorthand for the renamed categories, not an instruction to return chips to the map.
...
| Local-QA answering | Post-visit toast (decision #24): notification → non-blocking toast, binary "tourist-heavy spot? Yes/No" (rewritten 2026-07-30, decision #37 — was Local/Mix/Tourist) | Von Restorff; Fogg B=MAT; §2 ask-once ethics | Real signal at the freshest moment, no obligation | Tertiary, push-triggered · 1 tap to open, 1 (or ignore) to answer |
...
**Passport now has a real row, above — it's V1 scope, not a parked candidate.** Pulled forward 2026-07-30 (decision #31), reversing this table's prior "no row added, would land Tertiary if it ever shipped" placeholder — the tier call it was already carrying (Tertiary: opt-in, low-frequency, doesn't block the core loop) is confirmed for real now, not just a hypothetical. See §4 Journey 7 (promoted out of §8's parked list into the main V1 journey set) and §8. **Both items this promotion surfaced are resolved, 2026-07-30:** (1) the housing "Profile" tab name is a confirmed naming exception to the no-profiles gate, not a reopening (decision #39); (2) the per-Hood "Local" status mechanic **replaces** the seven-tier ladder outright — the ladder does not ship (decision #40).
...
- **Modal/sheet vs. persistent-chrome stacking (added 2026-07-30, T-032 `design-approval` REJECT finding 2 — centralizing so T-035/T-036/T-037/T-038 don't re-derive it).** Two different classes of persistent chrome behave differently when a nav-row modal opens: **(1) the nav row itself** must stay reachable the entire time a modal is open — direct nav-button switching (§2.1's exclusivity rule above) depends on it, so it renders in a stacking layer *above* the modal card, never covered. **(2) Other persistent Primary chrome anchored in the same bottom region** — near-me (L44) and the Places icon (L46) today, and any future control a later spec anchors in this same footprint — has no such requirement, so the correct default is: **fade out while a modal covers its position, fade back in the instant the modal closes**, honoring Reduce Motion same as the modal's own transition. Don't default to "same treatment as the nav row" for bucket (2) — it's a different question with a different answer, and rendering it underneath/through an open modal card (same z-tier, no hide behavior) is the failure mode this note exists to prevent. `design/phase-1/time-slider-design.md` §2 (Heat modal container row) is the worked example. Whoever specs the next bottom-anchored surface (T-036 Places, T-037 Profile, T-038 Search) should check which bucket their own persistent controls fall into rather than re-deriving this from scratch. **Ratified by Aviran, 2026-07-30** — this doc is Locked, so the addition above (made by `designer` mid-fix-pass, flagged for sign-off rather than assumed) is confirmed as intended, not a drift.
- **Map-surface interactivity behind an open sheet, the underlying mechanism (added 2026-07-30, resolving the gap the note above left open).** The general "map stays interactive" claim in the Search sheet row below (and T-033's Hood/place-detail spec before it) isn't just a UX preference — it needs a named implementation mechanism, or it silently fails to build (T-033 hit exactly this at `design-approval` before being fixed). The mechanism: SwiftUI's `.presentationBackgroundInteraction(...)` modifier, split by depth — enabled (`.enabled(upThrough:)` or `.enabled`) at shallow sheet depths where the map underneath should stay tappable, and left at its disabled default only at the deepest stacked depth (e.g. a place modal stacked over a Hood sheet), where a genuine modal block is correct. `design/phase-1/hood-place-detail-design.md` §2/§5/§7 is the worked example (T-033, code-review-verified). Whoever specs T-038 (Search sheet) should build the same mechanism against this row's claim rather than leaving it as an unspecified aspiration a second time.

**Additions, 2026-07-30 (founders-meeting reconciliation, decisions #27–36):**

- **Profile tab resolves the 3rd-button question.** The former "3rd button — TBD" placeholder (§9, old Q14) is now the **Profile button**, opening a tab that houses **Passport** (decision #31). Folds into the modal-exclusivity set above. **Resolved 2026-07-30 (decision #39):** "Profile" is confirmed as the final tab name, a naming exception to the no-profiles gate, not a reopening of it.
...
- **Saved vs. Been must read as different, not just be labeled differently.** Decision #30 requires manual "Saved" and dwell-triggered "Been" to stay visually/functionally distinct — this doc's existing provenance-label convention (a short word per Places-list row, §9 old Q11) already does that; renamed here from "Auto-saved" to **"Been"** to match the founders-meeting vocabulary (see §4 Journeys 3–4, §9 Q11). **Resolved 2026-07-30 (decision #38):** a place Apple Maps marks permanently closed can still be saved, shown with a distinct "permanently closed" badge — not folded into the tourist-trap flag.
- **Per-Hood "Local" status replaces the seven-tier ladder — resolved, not a reconciliation.** Decision #31 describes stamps accumulating toward "Local" status per designated Hood, with overall "Local" requiring it in every designated Hood. **Confirmed 2026-07-30 (decision #40): the per-Hood mechanic replaces the seven-tier global ladder (Tourist→Legend) outright** — the ladder does not ship alongside it. Journey 7 (§4) is rewritten to match.
- **Tourist-trap boolean fully replaces the three-way tag — applied, §6/`map-rendering-spec.md` rewritten.** Decision #28's boolean tourist-trap flag replaces decision #18's Local/Mix/Tourist tag entirely, **confirmed by Aviran 2026-07-30 (decision #37)** — not a narrower reading, the graduated tag is gone. §6's stroke/zoom rendering model is rewritten below against this.
- **Quick filters stay sheet-internal.** Decision #33 renames the categories (see §2 above); **confirmed 2026-07-30 (decision #41): decision #25 stands unreversed**, chips stay inside the search sheet under the new names.
...
**Journey 7, below, was a Phase 2/3 preview as of 2026-07-29 — that framing is now wrong.** The 2026-07-30 founders meeting pulled Passport forward into V1 outright (decision #31); Journey 7 is promoted here into the main journey set, out of §8's parked-features list. Fully resolved as of the same day (decisions #39, #40) — the journey is committed V1 scope with no open mechanics or naming questions left.
...
4. She taps a spot in the list — a rooftop bar with no tourist-trap flag, in a zone where heat is already climbing for this hour. Spot sheet opens: name, category, tourist-trap flag, save icon, Fast route / Scenic route (routing preview, locked 2026-07-29, §8a).
...
5. He finds a bar, not flagged a tourist trap, worth going to later, opens its spot sheet, and taps the save icon — it fills with a quick animation, an inline "Saved" confirmation appears, and he stays put.
...
flowchart TD
    ...
    F --> H[She taps the notification] --> I["Toast drops from top:<br/>Tourist-heavy spot? Yes / No"]
...
3. She taps the notification. Passenger foregrounds (if it wasn't already) to whatever it was last showing — no deep link into the spot's own sheet, since the toast itself carries everything needed to answer. A toast drops from the **top** of the screen: **"Does this feel like a tourist-heavy spot?"** — two tap targets, **Yes / No** (rewritten 2026-07-30, decision #37 — was a 3-way Local/Mix/Tourist ask). **Non-blocking, not modal:** it sits on top of whatever's underneath, dismissible by simply ignoring it, and auto-dismisses on its own after a few seconds if she doesn't touch it — consistent with every other "no modal, no interruption" moment in this doc, and with how an iOS banner already behaves.
4. She taps **No**. The toast collapses into a one-line "Thanks — that's shared with other travelers" and disappears.
...
*Either traveler from the journeys above, sometime after Journey 4's post-visit moment.*
...
flowchart TD
    ...
    F --> G["Per-Hood Local progress<br/>+ sticker album, per city<br/>passport-book UI<br/>(no separate global ladder — decision #40)"]
```
...
3. Her sticker count contributes toward **per-Hood "Local" status (decision #31):** stamps accumulate toward "Local" status within a set of Hoods Passenger designates; overall "Local" status requires reaching Local in *every* designated Hood, not just one. **Resolved 2026-07-30 (decision #40):** this replaces the old seven-tier global ladder (Tourist → Wanderer → Regular → Local → Insider → Native → Legend) outright — the ladder does not ship. Per-Hood Local status is the whole progression system. **[open, unchanged]** Whether a level-up gets any in-the-moment surfacing or stays silent until she next opens Passport is still undesigned.
4. **Later, opt-in:** she taps the **Profile button** (§2, §5) into the **Passport screen** — a private, single-user Tertiary surface. She sees her per-Hood Local progress and her sticker album for this city, passport-book UI. Nobody else ever sees this screen — no friend graph, no following, no social surface of any kind. **Confirmed 2026-07-30 (decision #39):** the tab housing this screen is named "Profile," literally — a naming exception to the standing no-profiles gate, not a reintroduction of accounts.
   - **Not designed here:** the old ladder's floated "Legend tier unlocks submitting your own recommendations" idea no longer applies now that the ladder doesn't ship — if a similar unlock is wanted for a per-Hood milestone instead, that's a fresh call, not an assumed carry-over.
5. **Outcome:** a personal, private record of real visits turned into a collectible and a per-Hood progression status — same underlying signal Journey 4 already uses, at zero extra detection cost. **This journey is committed V1 scope** (decision #31), fully resolved as of 2026-07-30 (decisions #39, #40) — no open mechanics questions or naming tension remain.
...
**Per note 2:** the requirement was the user can tell tourist-vs-not. As of 2026-07-30 that's a **boolean tourist-trap flag** (decision #37, confirmed by Aviran live the same day), not the three-way Local/Mix/Tourist tag this section described through 2026-07-29. Everything below is rewritten against the boolean model — this is the second real revision of the *rendering channel* (badge → stroke), but the first change to what the tag itself *is*.

**Resolved, 2026-07-30 (decision #37) — supersedes the flag that stood here.** The 2026-07-29 founders-meeting brief's boolean tourist-trap flag fully replaces the three-way tag, confirmed live: not a narrower reading, not "boolean alongside a retained tag" — the graduated Local/Mix/Tourist read is gone. This changes what local-QA (decision #24, Journey 4) verifies too: the toast becomes a binary "is this a tourist-heavy spot?" ask, not a 3-way classification. Public copy for the flag is **"tourist-heavy spot"** (decision #42, decision #36's copy flag resolved) — never "tourist trap" in user-facing text. `design/map-rendering-spec.md` §§2–3 are rewritten to match; this section states the resulting rule for the flows doc.

**Revised this round (2026-07-29 context, still the reason the channel is a stroke, not a badge).** Aviran's pushback, verbatim: *"how do you show tag layer on every location on the map? its gonna be too much information on one layer."* He was right — the previous version of this section badged every close-zoom pin, then stacked a warning badge on top of that. Worst case (Florentin at 8pm, dozens of pins) was heat fill plus a tag badge plus sometimes a warning badge, once per pin, all at once. That doesn't survive contact with a real dense neighborhood. The fix — tag gets the same progressive disclosure heat already has, rendered as a zone outline stroke rather than a badge — holds unchanged under the boolean model; only what the stroke *means* changed, not the channel it rides on.

**Zoom levels, and where the flag actually renders — rewritten for the boolean model:**

- **City-wide** — heat as neighborhood-scale blobs only. **No tag stroke at this zoom.** With only a flag (not a positive "Local" signal worth surfacing), there's nothing informative to show this coarse — a flagged zone reads exactly the same reasoning that kept Mix silent under the old model: absence of a mark isn't itself informative until you zoom in. No word labels, no pins.
- **Neighborhood** — zone boundaries visible, heat as zone-level stepped-band fill. **Outline stroke only on flagged (tourist-trap) zones**, plus a word label at the centroid ("Tourist-heavy spot") — the only zoom where the word appears. Unflagged zones render with no stroke and no label — silent, the same as Mix was silent under the old model, so "not flagged" and "not yet rated" still read identically on the map and resolve on tap, same convention as before. Still no spot pins.
- **Close** — individual spot pins appear. The zone's outline stroke (flagged zones only) persists at whatever boundary is still in view (no label at this zoom). **Pins themselves still carry no flag signal at all** — heat continues exactly as it rendered at neighborhood zoom, and pins exist only to mark a location and its category, plus a ring accent for anything already in the viewer's Places list (§6, decision #26). Tapping a pin opens the spot sheet, where the flag renders as what it always was underneath the map: a line in a sheet, not a decoration competing with dozens of others for the same screen.

**[design call, mine, unchanged under the boolean model]** Whether spot pins should carry the flag at all, at any zoom, was left open for me to decide under the old model and the reasoning still holds: **no, never**. Aviran's complaint was about density of *signal*, not just density of *objects* — a pin whose shape/fill encodes the flag still asks the eye to individually parse every pin in a crowded block one at a time. The fix that survives Florentin at 8pm is the same one as before: keep the flag off spot pins entirely, onto the zone sheet's list, read one row at a time instead of scanned all at once spatially. If a future pass wants a per-pin flag back (e.g., a "hide tourist-trap spots" close-zoom mode as a deliberate feature), that's a new decision to make then, not a default to fall back into.

**Slider hours:** unchanged — heat is the only time-variant layer, the tourist-trap flag isn't, so a zone's stroke never moves when the slider does, regardless of which granularity it's currently rendering at. **Where the slider control itself lives changed 2026-07-29** (§2.1, §5): it's now inside the heat modal rather than always visible on the map, but the rendering rules in this section apply identically regardless of which hour the modal last set.

**Unflagged renders nothing — no stroke, no label — at any granularity.** With a boolean, "not flagged" is the default, unmarked state, exactly as Mix was under the old three-tag model — marking it would be clutter carrying zero information. A zone gets a stroke only when flagged. **[design call]** A zone with no curated data yet also renders blank, identically — the map surface doesn't try to distinguish "confidently not a tourist trap" from "not yet rated," and that's an accepted trade-off, not an oversight: tapping in always resolves which one it is, the same way every other empty state in this doc already resolves "no data here" on contact rather than pre-announcing it on the map.

**The packed + tourist-trap combination — replace, don't stack.** A zone that's simultaneously busy and flagged doesn't get two decorations — it gets **one stroke treatment, in a distinct warning form, replacing the plain flagged stroke outright** (and the label reflects it, at neighborhood zoom — "busy and tourist-heavy"). Never two elements competing for the same handful of square millimeters (design-principles.md §2, Von Restorff: only one "special" element per view). A busy but **unflagged** zone still never gets a warning treatment of any kind — the absence of a warning stays legible precisely because the warning exists for the flagged case, and that logic survives the move from a 3-way tag to a boolean unchanged.
...
flowchart TD
    ...
    ProfileBtn --> Passport["Passport screen — Tertiary (1 tap)\nper-Hood Local progress + sticker album per city\n(decisions #29, #31; no separate global ladder — decision #40)"]
    ...
    Places -.geofence + dwell, async.-> QA["Local-QA toast — Tertiary\nTourist-heavy spot? Yes/No — push-triggered,<br/>not navigated to (decision #24)\nRewritten 2026-07-30: binary, decision #37\n(was 3-way Local/Mix/Tourist)"]
    Sticker -.-> Passport
...
5. **Does the computed busy + tourist-trap warning stroke (§6, zone-level) need its own VoiceOver label?** It's a display-time computation, not a stored flag combination, so it won't inherit whatever label the plain flagged stroke already carries. **Recommendation:** give it an explicit label ("busy and tourist-heavy — worth a second look") rather than relying on VoiceOver to read heat and the flag separately and expecting the combination to be inferred. This has a second, sharper edge to it: §6's unflagged-is-silent rule means a sighted user reads "no stroke" as not-flagged, but VoiceOver can't read an absence — full detail and resolution in `design/map-rendering-spec.md`'s accessibility section, kept coherent with this question rather than answered twice.
...
13. **Stamp collection / Passport (§8, Journey 7) — pulled forward into V1 2026-07-30 (decision #31); this question list now reflects that.** The old seven-tier ladder is superseded by per-Hood Local status (decision #40, resolves what used to be Q20 below) — `strategy/passenger-strategy.md`'s parked "per-city flavor names for the tier ladder" open question is likely moot now that there's no global ladder to flavor, worth a fresh look whenever per-Hood status gets its own copy pass, not re-litigated here. (Naming and the points-system unification were resolved 2026-07-28, both in strategy.md, both noted at §8 above.) **The one remaining UX-only open item carried over from the Phase 2 draft, unresolved by the V1 promotion:** whether a stamp/sticker or per-Hood progress milestone fires alongside Journey 4's local-QA toast or as a separate moment, and whether it gets any in-the-moment surfacing at all — see Journey 7 step 1 (§4). "Profile" tab naming is resolved (decision #39, Q19 below).

14. **The 3rd nav button — fully resolved 2026-07-30 (was genuinely undecided as of 2026-07-29).** Three options were on the table: **(a)** drop it; **(b)** device-local settings only; **(c)** fully reopen V1 scope for accounts + the existing "Passport" Phase-2 concept. The 2026-07-29 founders-meeting brief answers this as **option (c)** — Passport is pulled forward into V1 and lives under this button (decision #31, §1, §2, §5, §7). **Confirmed 2026-07-30 (decision #39):** the housing tab's "Profile" name is a naming exception to the no-profiles scope gate, not a reopening of it — both the button and the naming are settled.
...
**From the founders-meeting reconciliation (`PAS-8`) — resolved 2026-07-30. Aviran confirmed all four live the same day (decisions #37, #39, #40, #41); Linear `PAS-6` is closed on these items.**

18. ~~Tourist-trap boolean vs. the three-way tag~~ — **resolved:** decision #28's boolean fully replaces decision #18's Local/Mix/Tourist tag (decision #37). §6 and `map-rendering-spec.md` §§2–3 are rewritten to the boolean model as of this pass.
19. ~~"Profile tab" naming/scope for Passport~~ — **resolved:** "Profile" is the confirmed, literal tab name, a naming exception to the no-profiles gate rather than a reopening of it (decision #39).
20. ~~Passport's per-Hood "Local" status vs. the seven-tier ladder~~ — **resolved:** the per-Hood mechanic replaces the ladder outright; the ladder does not ship (decision #40). Journey 7 (§4) is rewritten to match.
21. ~~"Quick filters" placement~~ — **resolved:** sheet-internal, decision #25 stands unreversed (decision #41). §2's category-chip note and §5's map-chrome bullet updated to match.

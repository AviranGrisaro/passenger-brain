...
**Status:** Draft — awaiting Aviran's read. **Routing preview and live events are now locked** — Aviran signed off and `strategy/passenger-strategy.md` was updated 2026-07-29 (§8a below reflects this). One item remains explicitly **pending**, not locked: the search-sheet layout recommendation (sheet-over-map vs. Aviran's original literal split-view, §2.1, §9) — a design-review suggestion Aviran hasn't ruled on yet. The place-detail button-hierarchy fix (Save/Scenic/Fast) is also a design-review recommendation, not yet confirmed by Aviran — treat as proposed until he signs off.

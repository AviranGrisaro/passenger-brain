...
13. **Stamp collection / Passport (§8, Journey 7) — two questions still parked in `strategy/passenger-strategy.md`'s own Open questions, not re-litigated here:** per-city flavor names for the tier ladder (explicitly deferred cosmetic idea, not level-logic); and whether this is the same mechanic as Phase 3's points system or two systems that coexist. (The **"Passport"** surface name instead of "profile" — "profile" was a literal scope-gate tripwire, §8 above — is now **confirmed by Aviran, 2026-07-28**, no longer open; see strategy.md.) Full text lives in strategy.md per the doc-ladder rule — not restated here.

...
**Date:** 2026-07-29 (revised per Aviran's live grilling session + design-principles review on nav/map/search/routing — see §1 addendum, §2.1, §5, §8a, §9)
**Status:** Draft — awaiting Aviran's read. Two items below are explicitly **pending**, not locked: the search-sheet pattern recommendation (§2.1, §9) and the routing/events scope changes, which need Aviran's sign-off on a separate `strategy/passenger-strategy.md` redline (§8a) before they're build-ready — this doc states them as proposed/pending, not as already-authorized strategy.
**Source:** `strategy/passenger-strategy.md` (2026-07-27) + `strategy/decisions.md` (decisions 18–26, 2026-07-27) + Aviran's 2026-07-29 live-chat decisions (`PROGRESS.md`, founder-direct)
...
**Addendum, 2026-07-29 — post-grilling + design-principles review.** The controls that reach off the map are **3 side-by-side nav buttons** (search, heat, and a third — still undecided, see §9) plus a **persistent 4th icon** for saved places, separate from the 3. These are not a tab bar visually, but per the design review they should be **built on native TabView/sheet-presentation primitives, not custom-built from scratch**. Heat and localness rendering itself never moves behind a tap — the map always shows it, 0 taps, unchanged from §2 below. Tapping the **heat button** doesn't gate the layer; it opens a small modal holding only the 12h time slider and layer toggles, so "busy right now in one glance" stays true even though the slider control now lives one tap in. **Only one modal is ever open at a time** — tapping a different nav button closes whichever modal is open and opens the new one, never stacking — but each modal's own transient state (a half-typed search query, a mid-drag slider position) **persists across a close/reopen** rather than resetting. Saved places keep rendering as pins directly on the map; the 4th icon opens the full list. Full table/diagram detail: §2.1, §5, §8a. Open item this doesn't resolve: what the 3rd button is (§9).
...
As of this doc's current locked scope, no in-app routing screen exists — V1 hands off to Maps/Waze. **This line is pending a strategy.md scope change** (routing preview, §8a) — if Aviran signs off on the redline, a route polyline renders on the map as part of the spot-sheet flow, but it's still not a separate in-app routing *screen*, so the depth rule (§5) doesn't change either way.
...
| Spot sheet | Name, category, tag, save, "Go" — **pending update, §8a**: once routing preview is signed off, becomes place info + Fast route (filled primary) + Scenic route (outline secondary) + Save (demoted to icon-only bookmark/heart), replacing the current 3-equal-weight framing | Hick's Law; Von Restorff (one filled primary action, not three equal ones — design-review fix, §8a) | One clear default action (go), with a real alternative and a lightweight save, not three equal choices | Secondary · 1–2 taps; **Go/Fast route exits Passenger to native Maps/Waze** (routing preview itself, if approved, stays in-app — only the actual turn-by-turn hand-off exits) |
...
**No row added for stamp collection / Passport.** These tables are V1-only, per this doc's own framing — and the feature is a parked Phase 2/3 candidate (strategy.md), not something being built now. If it ships, it would land **Tertiary**-shaped (opt-in, low-frequency, doesn't block the core loop), same tier as Local-QA answering above. See §8 for the full treatment and §4 Journey 7 for a preview walkthrough.
...
- **Modal exclusivity + state persistence** (see §1 addendum): only one of {search sheet, heat modal, Places list, and any future 3rd-button modal} is ever open at once; switching closes the old one and opens the new one; each modal's own in-progress state survives that switch (see §6's reconciliation for the one nuance around search's category filter).
...
- **Map chrome** — always on screen, never dismissed: heat/tag layers (always-on rendering, never gated by a button — 2026-07-29), fading title, near-me button, the neighborhood button (conditional, §6), and **the 3 nav-button icons themselves** (search, heat, 3rd-TBD) **plus the persistent 4th Places icon**. Only the icons live here permanently; what they open is one of the surfaces below. Category chips don't belong to this list (decision #25 — they live inside the search sheet).
- **Modals/sheets, opened by the nav row** — partial-height, swipe-down or tap-outside to dismiss, **or auto-closed when a different nav button is tapped (2026-07-29 — only one open at a time, no stacking)**: the **heat modal** (time slider + layer toggles only, new 2026-07-29), the **search sheet** (query + category chips), and the **Places list** (merged Saved/Visited, decision #26 — promoted from a floating Tertiary icon to one of the persistent 4 as of 2026-07-29, §2). Each of these three retains its own in-progress state across a nav-button-triggered close/reopen (§2.1); the zone sheet and spot sheet, reached by tapping *into* the map rather than a nav button, aren't part of this exclusivity rule — they still dismiss the ordinary way (swipe down / tap outside) and don't compete with the nav row for "only one open" status.
...
**"Go" is not a surface at all** — as currently locked. It's a system hand-off to native Maps/Waze, which exits Passenger entirely. Returning to Passenger afterward (backgrounding/foregrounding) drops the user back wherever iOS left off — typically the spot sheet or the map — Passenger doesn't reconstruct any state for this, because it never built a screen to leave in the first place. **Pending update, §8a:** if the routing-preview scope change is signed off, "Go" splits into Fast route / Scenic route, and tapping either draws a polyline preview *inside* the spot sheet's own space before any hand-off — still not a new surface/level (see depth rule below), and the actual exit to native Maps/Waze still only happens on a second, explicit tap.
...
Nothing in V1 needs a third in-app level — the one feature that would have required it, a full in-app Scenic View screen, is still Phase 2 (§8a) even under the pending routing-preview change, since the preview renders as a polyline *inside* the existing spot sheet rather than opening a new screen — and keeping the full in-app routing screen out of V1 is exactly what holds this rule at 2 instead of 3. Dismissing any sheet always returns exactly one level up.
...
**Updated 2026-07-29** to reflect the 3(4)-button nav model: heat/tag rendering stays always-on (unchanged); the time slider moved off the map into a heat-button modal; Places moved from a floating Tertiary icon to the persistent 4th icon; only one of {search sheet, heat modal, Places list, 3rd-button modal} is ever open at once (dashed "closes" edges below). The 3rd button is a placeholder pending §9 Q14.
...
flowchart TD
    ...
    Spot -.Go/Fast route, exits app.-> Handoff["Native Maps/Waze\n(outside Passenger)"]
    Spot -.Scenic route, pending §8a.-> Preview["Polyline preview in spot sheet\n(not a new screen)"]
    ...
    Search -.tapping HeatBtn closes this.-> HeatModal
    HeatModal -.tapping SearchBtn closes this.-> Search
```
...
**As currently locked in `strategy/passenger-strategy.md`, both bullets below still hold** — Scenic View and Live Events are Phase 2 candidates, not V1. **§8a immediately below describes a pending change to both**, decided by Aviran 2026-07-29 but not yet applied to strategy.md — it needs his sign-off on a separate redline before it's authorized. Until that redline is applied, treat the two bullets below as the current state and §8a as proposed.

### 8a. Pending V1 scope changes, 2026-07-29 — routing preview + live events (awaiting Aviran's strategy.md sign-off)

Two scope changes came out of the grilling session that pull Scenic View and Live Events forward from Phase 2 into V1 — **not asserted here as already-authorized strategy**; both need Aviran to review and apply the redline chief-of-staff produced against `strategy/passenger-strategy.md` (routing section + phasing section) before `product`/`architect` treat them as committed.

- **Routing preview, pending.** Not full in-app turn-by-turn — the app draws the route as a **polyline on the map for comparison** (scenic vs. fast), then "Go" hands off to native Maps/Waze for the actual walk. No in-app voice/rerouting. Free in V1, no paywall (monetize later if at all — differs from the current strategy's "Scenic View is subscription-gated when it ships" line, since it's no longer the same full-routing feature). This also carries the place-detail hierarchy fix flagged by design review: the spot sheet's Save/Scenic/Fast trio reads today as three equal-weight buttons with no hierarchy (an anti-pattern) — fixed to **one filled primary** (Fast route, effectively today's "Go"), **one outline secondary** (Scenic route), and **Save demoted to an icon-only bookmark/heart**, not a full button. Doesn't add an in-app navigational level (§5's depth rule stays at 2) since the polyline renders inside the existing spot sheet, not a new screen.
- **Live events layer, pending — and launch-blocking if approved.** A live events overlay ships in V1 alongside heat + tag (third Primary-tier map layer, same additive-chrome shape already described in the Live Events bullet below). The real risk: Aviran's decision makes this **launch-blocking** — V1 does not ship until `data-engineer` has a working live-events ingestion pipeline. This is a genuine timeline/dependency risk, not a UX detail — a scoping task for `data-engineer` has been raised (see chief-of-staff's PROGRESS.md entry / Linear) to size the ingestion build before this is treated as a committed V1 dependency rather than an aspiration.
- **Open, not decided in this doc:** whether the routing-preview scope change and the events-layer scope change land in the *same* strategy.md redline pass or are reviewed separately — up to Aviran when he reviews the redline.

### Current locked state (strategy.md, as of 2026-07-27) — supersede-pending per §8a above

Full end-to-end flows for both, worked out ahead of time so the structure doesn't need to be re-derived when Phase 2 actually starts, live in [`design/phase-2-flows.md`](./phase-2-flows.md) — kept as a separate doc on purpose, not folded in here, since this doc is what the scope gate depends on staying clean.

- **Scenic View (Phase 2)** replaces V1's "Go" hand-off with a full-screen in-app routing surface that favors interesting streets over the fastest path. This isn't just an addition — it changes what leaving the spot sheet *means*. In V1, tapping Go ends the in-app journey (an exit, nothing to reconstruct on return). With Scenic View, the user's journey continues inside Passenger during transit, which reopens two things V1 currently sidesteps entirely: what re-entering the spot/zone context looks like after arrival (V1 has no "you've arrived" moment, since the app was never watching), and the depth rule, which goes back to 3 levels (map → zone → spot → Scenic View) the moment this ships. Worth scoping alongside Phase 2's proximity intelligence (arrival card) — both concern the in-transit experience and would likely share build surface. **Note the pending change above (§8a) is a lighter-weight preview, not this full screen — if signed off, this bullet's full-screen version stays a (further) Phase 2 candidate on top of the V1 preview, not replaced by it.**
...
14. **The 3rd nav button — genuinely undecided, not this doc's call (added 2026-07-29).** Aviran flagged this as unresolved after the grilling session: "need to think about it." Three options on the table, none picked: **(a)** drop it — ship 2 buttons only (search, heat); **(b)** scope it to device-local settings only — no name, no login, no persisted profile, just permission status / notifications / units; **(c)** fully reopen V1 scope for accounts + the existing "Passport" Phase-2 concept, which would be a real scope-gate question (`CLAUDE.md`'s "no social features / no profiles" line) needing its own explicit call, not an assumed inclusion. Nothing in §2/§5 above should be read as assuming any of the three — the tables simply carry a "3rd — TBD" placeholder wherever the button would sit. **No recommendation offered here on purpose** — carrying the question forward as asked, not picking for him.
...
16. **Place-detail button hierarchy (§8a) is bundled with the routing-preview scope change, not separable from it in this doc.** The Save/Scenic/Fast hierarchy fix (one filled primary, one outline secondary, Save demoted to icon) only applies once routing preview itself is authorized — if Aviran declines the routing-preview scope change but still wants the hierarchy fix applied to the current Save/Go pair, that's a smaller, separable ask worth saying explicitly, since as written here the two travel together.

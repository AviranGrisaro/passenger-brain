...
### Round-4 acceptance — **ACCEPTED WITH NOTES** (`product`, 2026-08-07, `passenger-code 1595af6`)

The `.fixedSize(horizontal: false, vertical: true)` clamp ships. Round-4's own required verification is met on both halves, each confirmed by more than one gate.

**Product's original finding re-measured first-hand, not inherited.** Own isolated worktree pinned to `1595af6`, own dedicated simulator (`pas77-product-accept`, iPhone 17 / iOS 26.5) and `-derivedDataPath`, throwaway XCUITest, all deleted after. `PlaceDetailModal` / Suzana Yemenite Kitchen with no route origin (`RouteControls` renders `EmptyView` for `.idle`, `.noOrigin` and `.failed` alike — `RouteControls.swift:12` — so the no-origin-yet card is layout-identical to the permission-declined one; asserted in-test that no `Fast` button existed, rather than assumed): **card height 224.7pt of an 874pt window, card top at 649.3pt (74.3% of screen height), capped `ScrollView` measuring 207.7pt.** Measured from the live accessibility tree's own frames rather than by pixel scan, so it is falsifiable against the 480pt constant with no colour tolerance in the path. **Was a fixed 480pt with a 60.2%-of-card dead band; is now 224.7pt with the content-free run bounded by the card's own ~16pt padding (≈7% of card, well under criterion 5's ~25% bar).** This is now the fourth independent measurement of this exact case and they agree to within ~4pt: `ios-developer` ~224pt, `ios-code-reviewer` (pre/post screenshot pair), `qa` ~221pt, this pass 224.7pt.

**Criterion 5 now passes on both halves** for this surface — top edge at 74.3% (bar: at or below 40%) and dead band ≈7% (bar: ~25%). The round-3 record above, which passed the top-edge half and failed the dead-band half, is closed by this commit.

**Note 1 — carried-forward disclosed gap, not re-litigated.** `EventDetailModal`'s sparsest case (seed-0002) was never live-confirmed, after attempts by `ios-developer` (2) and `qa` (3). Accepted on the same reasoning `ios-code-reviewer` and `qa` each reached independently: `.fixedSize` is a generic, content-agnostic modifier applied identically in all three files, live-confirmed shrinking on two of them, and this exact file's cap-and-scroll behaviour is separately confirmed on long content. Product concurs — a third concurring judgment, not a new one. What is *not* claimed is a measured number for that case; if one is ever taken and disagrees, this acceptance is the thing to revisit.

**Note 2 — the reason that case is unreachable is now its own ticket.** `qa`'s new data point (the seed-0002 marker appeared and was tapped, but no detail sheet opened) is filed as `PAS-79`, deliberately parked behind `PAS-78`'s in-flight fix, which may explain or resolve it. The test seam, not the layout mechanism, is what blocked the measurement.

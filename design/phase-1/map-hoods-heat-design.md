...
**Task:** T-031 · **PRD:** [`prds/map-hoods-heat/map-hoods-heat.md`](../../prds/map-hoods-heat/map-hoods-heat.md) (Draft v2)
...
**Owner:** designer · **Date:** 2026-07-30 · **Status:** ready for `design-approval` (resubmitted)
...
**Revision note (2026-07-30, post `design-approval` REJECT):** `product` rejected the first pass on 4 blocking findings plus 5 required-in-same-pass fixes — in every blocking case the prose below was already correct and the mockup's actual CSS/JS did something else. Fixed the mockup to match this doc, tightened the prose that undersold or omitted the gap, and rebuilt against PRD Draft v2 (the tourist-trap exclusion reasoning changed, not the scope). Every fix is called out inline below and confirmed in `PROGRESS.md`.
...
App icon tap
  → Map renders (Tel Aviv, city-wide camera, heat + Hoods already visible, 0 taps)
      ...
      → [after the title has fully faded, ~3.2–3.5s in] Location — When In Use system prompt appears
          → The map underneath stays visible but is not interactive while the system dialog is up — this is standard iOS system-alert modality, not a Passenger design choice. Deliberately sequenced to start only after the title's fade-out completes (~3.2s), so the two attention-grabbing elements never visually overlap.
...
| **Heat area layer** | Stepped-band density fill per Hood, keyed to the current hour, **identical at every zoom** | Same `MapPolygon` shapes, `.foregroundStyle(bandColor(for:))` where `bandColor` maps a discrete `enum HeatBand { case none, quiet, moderate, busy }` to a fixed opacity step of one accent hue — never a computed gradient. **The band→opacity table is one fixed mapping with no zoom parameter at all** — city-wide, Hood, and Close all read the same `bandColor(for:)` call. No blur, feather, or soft-edge filter is ever applied to the fill; band boundaries follow Hood boundaries exactly at every zoom (PRD req. 4, tightened at `design-approval` 2026-07-30). Coarser granularity at city-wide, if wanted later, is a Hood-geometry decision (fewer/larger shapes), never a rendering-layer blur standing in for "zoomed out." `HeatBand` is exactly the `DensityContract.swift` idea `SALVAGE.md` flags REUSE: one enum, no blended score. |
| **Location permission flow** | Lazy When-In-Use ask, granted/denied handling | `CLLocationManager` wrapped in an `@Observable` `LocationStore`. Call `requestWhenInUseAuthorization()` only once, only when `authorizationStatus == .notDetermined`, triggered after the map's first `onAppear` and after the cold-open title has fully faded (~3.2–3.5s in — see §1) so the two never visually overlap; not at launch, not gated behind a tap (per `ux-flows.md` §9 Q2's recommendation, which this spec treats as the working default; the exact trigger delay is still an open item for Aviran to confirm per that doc, only the "don't collide with the title" constraint is locked here). The system alert is modal like any real iOS permission prompt — the map stays visible underneath but is not interactive while it's up; it is never rendered as a custom in-app sheet. **Purpose string must describe V1 accurately**: recentering the map and showing a "you are here" marker — never phrasing implying personalized or location-based crowd data, since density in V1 is a synthetic, Hood-level feed (decision #4), not derived from the user's own location. Granted state drives `MapCameraPosition.userLocation()` for the recenter and shows the system-provided user-location dot; denied state is read once from `CLLocationManager.authorizationStatus` and never re-requested. |
...
| Near-me button (P1) | Recenter control | Standard circular icon button, bottom-third placement (Thumb Zone, design-principles.md §3). Disabled visual state when permission is denied. Tapping it while denied surfaces a real, visible inline copy element anchored just above the button — not a caption that only exists in reviewer-facing text — reading roughly "Location is off. Turn it on in Settings to use Near Me," auto-dismissing after a few seconds; it never re-triggers the system dialog. |
| Hood name label (P1) | Visible at Hood zoom and closer only. **Name only — no density word ever renders here.** | `Text` anchored near each Hood's rendered centroid, hidden at city-wide zoom — mirrors `map-rendering-spec.md` §2's zoom-gated disclosure pattern, applied here to the name label since heat itself (unlike the excluded tourist-trap stroke) is visible at every zoom per PRD req. 4. **The centroid carries the name and nothing else visible.** The fill is the density signal for sighted users; a second visible word ("quiet"/"moderate"/"busy") at the same centroid is not authorized by the PRD or this table and collides with the centroid word-label channel `map-rendering-spec.md` §2/§3 reserve for the tourist-trap flag (`prds/tourist-trap-flag/tourist-trap-flag.md` req. 3) — stacking two word labels at one point is exactly the stacked-signal problem the north star's "two orthogonal layers" framing rules out. VoiceOver gets the spoken density word instead (§4) — it can't perceive an absence of fill, so the word has to be said, not shown. |
...
| **Permission-denied** | Map stays at the default Tel Aviv city-wide camera, fully pannable/zoomable/tappable. Near-me button greys out; tapping it surfaces a real, visible inline copy element (not just a description of one) anchored above the button — "Location is off. Turn it on in Settings to use Near Me" — that auto-dismisses after a few seconds, instead of re-prompting (iOS won't re-trigger the system dialog once denied, and the app shouldn't pretend otherwise). No degradation to Hood or heat rendering — those never depended on location. |
...
| P0-1 (sub-bullet) Cold launch to an interactive map under 3s on the oldest supported device | **Not resolved in this spec.** This is a build-time performance target, not a design-time layout/state decision — flagged for `architect`/`ios-developer` to verify against real device hardware and MapKit initialization cost, not claimed as covered by any design choice here. *(Row added at `design-approval` 2026-07-30 — previously claimed-covered with no actual row.)* |
...
| P0-4 Heat area rendering (stepped bands, no gradients, no blur/feather at any zoom, 0-tap visible, same band = same opacity at every zoom, never a blended score) | §2 Heat area layer row (now states explicitly: one fixed band→opacity table, no zoom parameter, no blur filter); §4's color-alone note |
| P0-5 Heat bound to one hour | §2 Density feed client row (the `selectedHour` handoff seam for T-032) |
| P0-6 Lazy location permission | §2 Location permission flow row (now states the system alert is modal, map visible but not interactive underneath, and gives the accurate purpose-string content); §1 Flow; §3 Permission-denied state |
| P0-7 Degraded data is silent (no on-map text of any kind, at any zoom) | §3 Empty and Error states; §2 Hood name label row (no density word ever renders at the centroid, including for a gapped Hood) |
| P1 Near-me recenter | §2 Near-me button row (real inline Settings-copy element, not a reviewer-only caption) |
| P1 Hood name label at neighborhood zoom — name only, no density word | §2 Hood name label row |
...
- Cold-open sequence — fading title, then the lazy system permission prompt appearing only after the title has fully faded (so the two never overlap), never before the map is already interactive ("Replay cold open" replays this).
- Zoom-level control (City-wide / Hood / Close) — heat renders as the **same crisp stepped-band fill at every zoom level**, identical opacity per band regardless of zoom (no blur filter, no per-zoom opacity clamp); only the name label's visibility is zoom-gated, per `map-rendering-spec.md` §2's zoom-gated disclosure pattern applied to the name. No density word ever appears at a Hood centroid — the fill carries that signal, and the centroid word-label channel stays reserved for the incoming tourist-trap flag.
- Permission control (Not asked / Granted / Denied) — recenter + "you are here" marker on Granted; greyed near-me + a real inline "turn on Location in Settings" copy element (anchored above the button, auto-dismissing) on tap while Denied.
...
4. **Permission-prompt exact timing** — this spec follows `ux-flows.md` §9 Q2's recommendation (auto-prompt a couple of seconds after first render) as the working default; the exact delay is still open for Aviran to confirm, not yet a locked decision. **What is locked as of this revision:** the prompt must not fire while the cold-open title is still visibly fading (~120ms–3.2s after launch) — the mockup now sequences it to ~3.2–3.5s so the two never visually overlap. Any final timing `ios-developer` picks must preserve that non-overlap constraint even if the absolute delay changes.
5. **P0-1's "<3s cold launch on the oldest supported device"** — a real performance target, not resolved by this design pass (§5 traceability table says so explicitly rather than claiming coverage). `architect`/`ios-developer` need to verify this against actual MapKit init cost and Hood-geometry payload size on the oldest supported device.

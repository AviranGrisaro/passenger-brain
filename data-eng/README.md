...
- `scenic-walk-tiktok-feasibility.md` — PAS-7 scoping (Scenic Walk weighted routing + TikTok place extraction + Google Maps import read). Full derivation lives here; feature-scoped summaries with an explicit go/no-go/slip call, ported into the PRD-gate location `product` reads, are at `prds/scenic-walk/feasibility.md` and `prds/tiktok-import/feasibility.md` (2026-07-30) — update both together if either changes.

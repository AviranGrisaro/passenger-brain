...
Before generating a new plan, check `content/10-outputs/` for an existing plan from today.

**If a same-day plan exists:**
```
I see you already have a plan for today (`content/10-outputs/[filename]`).
...
1. `content/strategy/` - Quarter priorities, OKRs, North Star
2. `content/10-outputs/weekly-plans/` - This week's priorities (if `/weekly-plan` was run)
3. `content/prds/` - Active PRDs and their stages
4. `content/01-discovery/stakeholder-*.md` - Stakeholder profiles and communication styles
5. `content/10-outputs/meeting-notes/` - Recent meeting context
6. `content/10-outputs/launches/` - Recently launched features (past 2 weeks)
...
2. **Check for yesterday's plan (carry-over):**
   - Read `content/10-outputs/daily-plans/` for yesterday's plan file
...
3. **Check for weekly plan:**
   - Read `content/10-outputs/weekly-plans/YYYY-WXX-weekly-plan.md` for current week
...
For each meeting:
- Look up attendees in `content/01-discovery/stakeholder-*.md`
- Scan `content/10-outputs/meeting-notes/` for recent interactions with each person
...
Cross-reference with `content/strategy/`:
...
- Scan `content/10-outputs/meeting-notes/` for unchecked action items
...
Check `content/10-outputs/launches/` for features launched in past 2 weeks.
...
If MCP not available:
- Check if metrics file exists in `content/07-analytics/`
...
1. **Profile lookup:**
   - Read `content/01-discovery/stakeholder-*.md` if exists
   - Extract: Role, communication style, priorities, pet peeves

2. **Recent interaction history:**
   - Scan `content/10-outputs/meeting-notes/` for past meetings with this person
...
Create file: `content/10-outputs/daily-plans/YYYY-MM-DD-daily-plan.md`
...
- PM's role is VP, Director, or Head of [function] (check `content/01-discovery/personal-context-pm-background.md` or `content/01-discovery/business-info.md`)
...
- [ ] **File saved:** Plan saved to `content/10-outputs/daily-plans/YYYY-MM-DD-daily-plan.md`
...
4. **Save as:** `content/10-outputs/daily-plans/YYYY-MM-DD-draft.md`
...
- Check PRD X in `content/prds/`
- Find Feature Z in `content/10-outputs/launches/`
...
- Suggest: "Want richer meeting context? Fill out stakeholder profiles in `content/`"

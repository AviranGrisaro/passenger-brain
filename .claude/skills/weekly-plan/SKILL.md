...
1. `content/strategy/` - Quarter OKRs, North Star, strategic pillars
2. `content/10-outputs/weekly-reports/` - Last week's review (if exists)
3. `content/prds/` - Active PRDs and their stages
4. `content/10-outputs/launches/` - Upcoming launches
5. `content/10-outputs/weekly-plans/` - Previous weekly plans (for pattern analysis)
...
2. **Check if week already planned:**
   - Look for `content/10-outputs/weekly-plans/YYYY-WXX-weekly-plan.md`
...
Check for `content/10-outputs/weekly-reports/[last-week]-weekly-review.md`:
...
Create file: `content/10-outputs/weekly-plans/YYYY-WXX-weekly-plan.md`
...
- Suggest: "Fill out `content/strategy/OKRs.md` for better alignment"
...
- [ ] **Context was checked:** Reviewed `content/strategy/` for quarter OKRs, `content/prds/` for active PRDs, and `content/10-outputs/meeting-notes/` for upcoming commitments
...
- [ ] **Carry-over items from last week addressed:** If `content/10-outputs/weekly-reports/` or `content/10-outputs/weekly-plans/` contain incomplete items from last week, they are explicitly acknowledged as carried over, deferred, or dropped with reasoning

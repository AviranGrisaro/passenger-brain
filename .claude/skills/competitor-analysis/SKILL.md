...
**Output:** `content/10-outputs/research-synthesis/competitor-analysis-[name]-[date].md`
...
|--------|---------------|--------------|-----------------|
| User Research | `content/11-competitors/*.md` | competitor name, "switched to", "chose", "vs [competitor]", "competitor" | Customer quotes, pain points, feature comparisons |
| Existing Analysis | `content/11-competitors/competitive-*.md` | competitor name | Past findings, dates, trends, avoid duplication |
| Meeting Notes | `content/10-outputs/meeting-notes/*.md` | competitor name, "lost deal", "churn", sales, CS | Sales losses, CS feedback, win/loss patterns |
| PRDs | `content/prds/*.md` | competitor name, "competitive", "positioning" | Feature decisions, positioning rationale |
| Strategy | `content/strategy/*.md` | competitor name, "positioning", "differentiation" | Strategic context, counter-positioning |
| Metrics | `content/07-analytics/*.md` | "churn", "retention", competitor name | Churn to competitors, competitive benchmarks |
...
Surface prior competitor learnings before researching:

```bash
bash /Users/avirang/Documents/amp/scripts/learnings-search.sh --skill competitor-analysis --limit 10
```

If a specific competitor was named, also search by query:
```bash
bash /Users/avirang/Documents/amp/scripts/learnings-search.sh --query <competitor-name> --limit 5
```

If a returned learning applies, prefix your analysis with **"Prior learning applied: [key] (confidence N/10)"** so Aviran sees how the competitive picture has evolved.
...
**Checking:**
- `content/11-competitors/` for user interviews mentioning competitors
- `content/11-competitors/competitive-*.md` for past competitive analysis
- `content/10-outputs/meeting-notes/` for sales/CS notes with competitive intel
- `content/prds/` for competitive positioning decisions
- `content/strategy/` for strategic context
- `content/07-analytics/` for churn data
...
**Output:** Comprehensive report saved to `content/10-outputs/research-synthesis/competitor-analysis-[date].md`
...
**Output:** Updates saved to `content/10-outputs/research-synthesis/competitive-intel-[month].md`
...
Save to: `content/10-outputs/research-synthesis/competitive-intel-[month].md`
...
**Deep Analysis:**
- Active work: `content/10-outputs/research-synthesis/competitor-analysis-[name]-[date].md`
- When finalized: Move to `content/11-competitors/` for future reference

**Ongoing Monitoring:**
- Save directly to: `content/10-outputs/research-synthesis/competitive-intel-[month].md`
...
Log any competitor insight discovered during this run that would save 5+ minutes next time. Schema: `content/01-discovery/learnings-schema.md`.

```bash
bash /Users/avirang/Documents/amp/scripts/learnings-log.sh \
  --skill competitor-analysis \
  --type strategic \
  --key <competitor-slug>-<insight-tag> \
  --insight "1-2 sentence finding about the competitor's positioning, weakness, or move." \
  --confidence <1-10> \
  --source <observed|inferred> \
  --project-or-feature <passenger-feature-slug if relevant, else omit> \
  --files content/11-competitors/<file>.md
```

Good things to log: a competitor's strategic bet now confirmed by their roadmap; a pricing change that signals positioning shift; a feature gap that explains your team's churn pattern; a market segment they've abandoned. **Skip:** generic "they have X feature" observations — those belong in the analysis doc, not as a learning.

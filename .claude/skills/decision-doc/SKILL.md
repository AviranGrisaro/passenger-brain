...
Surface prior decisions on adjacent topics:

```bash
bash /Users/avirang/Documents/amp/scripts/learnings-search.sh --skill decision-doc --limit 10
```

If the user has named the topic, also search by it:
```bash
bash /Users/avirang/Documents/amp/scripts/learnings-search.sh --query "<topic-keyword>" --limit 5
```

If a returned learning is relevant (e.g., a similar build-vs-buy choice was made before; a prior trade-off has now been validated or disproven), prefix that section with **"Prior learning applied: [key] (confidence N/10)"**.
...
**Source:** `content/strategy/`, `content/prds/`
...
**Source:** `content/01-discovery/stakeholders.md` + profiles
...
**Source:** `content/10-outputs/decisions/`
...
**Source:** `content/07-analytics/`, this chat thread
...
**Source:** `content/11-competitors/competitive-*.md`, web search if needed
...
Decisions are the highest-leverage thing to learn from. Log the lasting insight from this decision — not the decision itself (the doc captures that), but the **pattern or trade-off** worth applying next time. Schema: `content/01-discovery/learnings-schema.md`.

```bash
bash /Users/avirang/Documents/amp/scripts/learnings-log.sh \
  --skill decision-doc \
  --type <pattern|strategic|pitfall> \
  --key <decision-topic-slug> \
  --insight "The trade-off pattern or strategic principle this decision validated/disproved." \
  --confidence <1-10> \
  --source <observed|user-stated|cross-model> \
  --project-or-feature <passenger-feature-slug if applicable> \
  --files content/10-outputs/decisions/<decision-doc>.md
```

Good things to log: a build-vs-buy trade-off resolved (and why); a stakeholder type whose objection always means "scope creep" not "real risk"; an approach that turned out smaller than estimated (or larger). **Calibration loop:** when this decision is revisited in 90 days, log the outcome as a new entry with `--type pattern` to confirm or reverse the original insight.
...
- [ ] **File saved to correct location:** Output saved to `content/10-outputs/decisions/decision-[topic]-[date].md`
- [ ] **Context routing table was checked:** Reviewed `content/10-outputs/decisions/` for past decisions, `content/strategy/` for alignment, and `content/01-discovery/stakeholders.md` for stakeholder context
...
- [ ] **Conflicts with existing strategy or past decisions flagged:** Any tension with decisions in `content/10-outputs/decisions/` or goals in `content/strategy/` is called out with explanation of how the conflict is resolved

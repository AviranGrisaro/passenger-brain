...
# State file location (no gstack dependency)
STATE_DIR="${CLAUDE_PLUGIN_DATA:-$HOME/.claude/state/passenger-safety}"
FREEZE_FILE="$STATE_DIR/freeze-dir.txt"

# If no freeze file exists, allow everything (not yet configured or after /unfreeze)
...
case "$FILE_PATH" in
  ...
  *)
    ...
    printf '{"permissionDecision":"deny","message":"[freeze] Blocked: %s is outside the freeze boundary (%s). Only edits within the frozen directory are allowed. Run /unfreeze to widen scope."}\n' "$FILE_PATH" "$FREEZE_DIR"
    ;;
esac

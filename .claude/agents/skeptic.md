...
- GOOD: "Passenger has no users yet — Phase 1 is the test of whether anyone reopens it. What specific task does a specific user currently waste 10+ minutes on per week that this eliminates? Name the user — a real one from `feedback/`. If you can't name one, the segment isn't validated."
...
Source: gstack `/office-hours` anti-sycophancy patterns (`office-hours/SKILL.md:956-996`), adapted for Aviran's PRD reviews on Passenger.
